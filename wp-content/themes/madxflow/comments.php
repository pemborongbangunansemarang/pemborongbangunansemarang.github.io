...
<div id="comments" class="tf_module_comments" itemscope itemtype="http://schema.org/UserComments">
	...
	<?php if ( post_password_required() && have_comments() ) : ?>
		...
		<p class="nopassword"><?php _e( 'This post is password protected. Enter the password to view any comments.', 'madx-flow' ); ?></p>

	<?php elseif ( have_comments() ) : ?>

		<h3 class="comments-title"><?php comments_number(__('No Comments','madx-flow'), __('One Comment','madx-flow'), __('% Comments','madx-flow') );?></h3>
	...
	<?php if ( comments_open() ) : ?>
		...
		<?php
			...
			'author' => '<p class="comment-form-author">' .
					...
					'<label for="author">' . __( 'Your Name' , 'madx-flow' ) . '</label> ' .
			...
			'email'  => '<p class="comment-form-email">' .
					...
					'<label for="email">' . __( 'Your Email' , 'madx-flow' ) . '</label> ' .
					( $req ? '<span class="required">*</span>' : '' ) .
					'</p>',
			'url'    =>  '<p class="comment-form-url">' .
					'<input id="url" name="url" type="text" value="' . esc_attr(  $commenter['comment_author_url'] ) . '" size="30"' . $aria_req . ' />' .
					'<label for="website">' . __( 'Your Website' , 'madx-flow' ) . '</label> ' .
					'</p>') ),
			'comment_field' => '<p class="comment-form-comment">' .
					'<textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" class="required"></textarea>' .
					'</p>',
			'logged_in_as' => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%s">%s</a>. <a href="%s">Log out?</a>', 'madx-flow' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( get_the_ID() ) ) )	) . '</p>',
			'comment_notes_before' => '',
			'comment_notes_after' => '',
			'cancel_reply_link' => __( 'Cancel' , 'madx-flow' ),
			'label_submit' => __( 'Post Comment' , 'madx-flow' ),
		);
