...
class TF_Update_Check {
	...
	public function __construct( $name, $version ) {
		...
		$this->versions_url = '';
		$this->package_url = "https://madxartwork.net/files/{$this->name}/{$this->name}.zip";
		...
		// Executes madx_updater function using wp_ajax_ action hook
	...
	function tf_upgrade_complete( $update_actions, $theme ) {
		if ( $theme == $this->name ) {
			$update_actions['themes_page'] = '<a href="' . esc_url( add_query_arg( array( 'page' => $this->page, 'tf_updated' => 'true' ), self_admin_url( 'admin.php' ) ) ) . '" title="' . __( 'Return to Flow Settings', 'madx-flow' ) . '" target="_parent">' . __( 'Return to Flow Settings', 'madx-flow' ) . '</a>';
		}
	...
	function dialog() {
		...
		<div class="prompt-box">
			<div class="show-login">
				<form id="madx_update_form" method="post" action="<?php echo esc_url( $action_url ); ?>">
					<p class="prompt-msg"><?php _e( 'Enter your madx login info to upgrade', 'madx-flow' ); ?></p>
					<p><label><?php _e( 'Username', 'madx-flow' ); ?></label> <input type="text" name="username" class="username" value=""/></p>
					<p><label><?php _e( 'Password', 'madx-flow' ); ?></label> <input type="password" name="password" class="password" value=""/></p>
					<input type="hidden" value="<?php echo esc_attr( $login ); ?>" name="login" />
					<p class="pushlabel"><input name="login" type="submit" value="<?php _e( 'Login', 'madx-flow' ); ?>" class="button tf-upgrade-login" /></p>
				</form>
			</div>
			<div class="show-error">
				<p class="error-msg"><?php _e( 'There were some errors updating the theme', 'madx-flow' ); ?></p>
			</div>
		</div>
	...
	function tf_notify_elsewhere() {
		...
		if ( ! empty( $notice ) && is_array( $notice ) && ! empty( $notice['version'] ) ) {
			printf( '<p class="update-nag">' . __( '%s version %s is now available.', 'madx-flow' ), $notice['theme'], $notice['version'] );
			if ( current_user_can( 'update_themes' ) ) {
				printf( ' ' . __( 'Go to <a href="%s">Flow Settings</a> to update.', 'madx-flow' ), add_query_arg( 'page', 'tf-settings', admin_url( 'admin.php' ) ) );
			} else {
				echo ' ' . __( 'Notify your site administrator that the update is ready.', 'madx-flow' );
			}
			echo '</p>';
		}
	}
	...
	public function check_version( $return = '' ) {
		...
		if ( is_object( $newUpdate ) && $time_not_changed ) {
			if ( version_compare( $this->version, $newUpdate->version, '<') ) {
				$notice .= sprintf( '<div class="update-nag update %s">' . __( '%s version %s is now available. <a href="%s" title="" class="%s" target="%s" data-theme="%s" data-package_url="%s" data-nicename_short="%s" data-update_type="%s">Update now</a> or view the <a href="%s" title="" class="madx_changelogs" target="_blank" data-changelog="%s">changelog</a> for details.', 'madx-flow') . '</div>',
			...
			if ( version_compare( $this->version, $remote_version, '<' ) ) {
				...
				$notice .= sprintf( __('<div class="update update-nag %s">%s version %s is now available. <a href="%s" title="" class="%s" target="%s" data-theme="%s" data-package_url="%s" data-nicename_short="%s" data-update_type="%s">Update now</a> or view the <a href="%s" title="" class="madx_changelogs" target="_blank" data-changelog="%s">changelog</a> for details.</div>', 'madx-flow'),
					esc_attr( $new->login ),
					$this->nicename,
					$new->version,
					esc_url( $new->url ),
...
//		$xml->loadXML( trim( $body ) );
	...
	public function enqueue() {
		...
		wp_localize_script( 'tf-updater', 'tfUpdater', array(
			'confirm_reset_styling'	=> __( 'Are you sure you want to reset your theme style?', 'madx-flow' ),
			'confirm_reset_settings' => __( 'Are you sure you want to reset your theme settings?', 'madx-flow' ),
			'confirm_refresh_webfonts'	=> __( 'Are you sure you want to refresh the Google Fonts list? This will also save the current settings.', 'madx-flow' ),
			'confirm_update' => __( 'Make sure to backup before upgrading. Files and settings may get lost or changed.', 'madx-flow' ),
			'confirm_delete_image' => __( 'Do you want to delete this image permanently?', 'madx-flow' ),
			'invalid_login' => __( 'Invalid username or password.<br/>Contact <a href="https://madxartwork.net/contact">madx</a> for login issues.', 'madx-flow' ),
			'unsuscribed' => __( 'Your membership might be expired. Login to <a href="https://madxartwork.net/member">madx</a> to check.', 'madx-flow' ),
			'enable_zip_upload' => sprintf(
				__( 'Go to your <a href="%s">Network Settings</a> to enable <strong>zip</strong>, <strong>txt</strong> and <strong>svg</strong> extensions in <strong>Upload file types</strong>  field.', 'madx-flow' ),
				esc_url( network_admin_url('settings.php').'#upload_filetypes' )
			),
			'filesize_error' => __( 'The file you are trying to upload exceeds the maximum file size allowed.', 'madx-flow' ),
			'filesize_error_fix' => sprintf(
				__( 'Go to your <a href="%s">Network Settings</a> and increase the value of the <strong>Max upload file size</strong>.', 'madx-flow' ),
	...
	/**
	 * Validate login credentials against madx's membership system
	 * 
	 * @since 1.0.0
	 */
	...
	function tf_updater() {
		...
		$upgrader = new TF_Upgrader( new Theme_Upgrader_Skin(
			array(
				'theme' => $slug,
				'title' => __( 'Update Flow', 'madx-flow' )
			)
		));
