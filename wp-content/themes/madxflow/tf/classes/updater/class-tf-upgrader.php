...
class TF_Upgrader extends WP_Upgrader {
	...
	function upgrade_strings() {
		$this->strings['up_to_date'] = __('The theme is at the latest version.', 'madx-flow');
		$this->strings['no_package'] = __('Update package not available.', 'madx-flow');
		$this->strings['downloading_package'] = __('Downloading update from <span class="code">%s</span>&#8230;', 'madx-flow');
		$this->strings['unpack_package'] = __('Unpacking the update&#8230;', 'madx-flow');
		$this->strings['remove_old'] = __('Removing the old version of the theme&#8230;', 'madx-flow');
		$this->strings['remove_old_failed'] = __('Could not remove the old theme.', 'madx-flow');
		$this->strings['process_failed'] = __('Theme update failed.', 'madx-flow');
		$this->strings['process_success'] = __('Theme updated successfully.', 'madx-flow');
	}
	...
	function download_url( $url, $timeout = 300 ) {
		...
		if ( ! $url )
			return new WP_Error( 'http_no_url', __( 'Invalid URL Provided.', 'madx-flow' ) );

		$tmpfname = wp_tempnam($url);
		if ( ! $tmpfname )
			return new WP_Error( 'http_no_file', __( 'Could not create Temporary file.', 'madx-flow' ) );
