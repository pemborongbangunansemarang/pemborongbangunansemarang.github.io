...
class TF_Ajax {
	...
	public function builder_update_template_part() {
		...
		$data = array(
			...
			'shortcode' => $shortcode,
			'edit_url'  => $edit_url,
			'caption'   => sprintf( __('Template Part: %s', 'madx-flow'), esc_html( $template_part->post_title ) ),
			'styles'    => $css,
		);
	...
	public function plupload() {
		...
		if ( $file && ! isset( $file['error'] ) ) {
			...
			if( in_array( $ext[1], $allowed_types ) ){
				...
				$url = wp_nonce_url('admin.php?page=madx-flow');
				...
				if( $wp_filesystem->exists( $filename ) ) {
					...
					$file['error'] = __('Data could not be loaded because import data not founded in the zip.', 'madx-flow');
