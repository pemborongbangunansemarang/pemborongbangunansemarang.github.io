...
class TF_Admin {
	...
	public function enqueue_scripts( $hook_suffix ) {
		...
		if ( ! ( in_array( $screen->id, array( 'toplevel_page_tf-settings', 'madx-flow_page_tf-themes', 'edit-tf_template', 'edit-tf_template_part', 'dashboard_page_tf-about', 'nav-menus' ) ) 
				||
		...
		// FontAwesome uses the same handle than madx Builder to prevent double loading the fonts
		wp_enqueue_style( 'tf-icon-font', $TF->framework_uri() . '/assets/css/fontawesome/css/font-awesome.min.css', array(), '4.3' );
		wp_enqueue_style( 'tf-icons', $TF->framework_uri() . '/assets/css/madx-icons/madx-icons.css', array(), $TF->get_version() );
		...
		$localize_app = array(
			...
			'module_delete' => __('Are you sure to delete this module?', 'madx-flow' ),
			'sub_row_delete' => __('Are you sure to delete this sub row?', 'madx-flow' ),
			'row_delete' => __('Are you sure to delete this row?', 'madx-flow' ),
			'drop_module_text' => __('drop module here', 'madx-flow' ),
			'row_option_title' => __('Row Options', 'madx-flow'),
			'base_path' => $TF->framework_path(),
			'base_uri' => $TF->framework_uri(),
			'template_type' => get_post_meta( get_the_ID(), 'tf_template_type', true ),
			'replace_confirm' => __('This import will override all current data. Press OK to continue.', 'madx-flow'),
	...
	public function register_menu_page() {
		...
		add_menu_page( __('madx Flow', 'madx-flow'), __('madx Flow', 'madx-flow'), 'manage_options', $this->slug, array( $this, 'render_settings' ), $TF->framework_uri() . '/assets/img/favicon.png', '61.3' );
		add_submenu_page( $this->slug, __( 'Flow Themes'), __('Flow Themes'), 'manage_options', 'tf-themes', array( $this, 'render_theme_page' ) );
		add_submenu_page( $this->slug, __( 'Flow Templates', 'madx-flow' ), __( 'Flow Templates', 'madx-flow' ), 'manage_options', 'edit.php?post_type=tf_template' );
		add_submenu_page( $this->slug, __( 'Flow Template Parts', 'madx-flow' ), __( 'Flow Template Parts', 'madx-flow' ), 'manage_options', 'edit.php?post_type=tf_template_part' );
		add_submenu_page( $this->slug, __( 'Global Styling', 'madx-flow' ), __( 'Global Styling', 'madx-flow' ), 'manage_options', 'admin.php?page=styling-panel-frontend' );
		add_submenu_page( $this->slug, __( 'Custom CSS', 'madx-flow' ), __( 'Custom CSS', 'madx-flow' ), 'manage_options', 'admin.php?page=custom-css-frontend' );

		if ( isset( $submenu['tf-settings'][0][0] ) ) 
			$submenu['tf-settings'][0][0] = __('Settings', 'madx-flow');

		// About page
		add_dashboard_page(
			__('Welcome to madx Flow', 'madx-flow'),
			__('Welcome to madx Flow', 'madx-flow'),
			'read',
			'tf-about',
	...
	public function register_settings() {
		...
		register_setting( 'tf-settings', 'madx-flow' );
	...
	public function render_settings() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'You do not have sufficient permissions to access this page.', 'madx-flow' ) );
		}
		?>
		<div class="wrap madx-flow-settings">
			<h2>
				<?php _e( 'madx Flow Settings', 'madx-flow' ); ?> 
			</h2>
