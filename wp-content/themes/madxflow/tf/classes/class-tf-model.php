...
class TF_Model {
	...
	public static function template_types() {
		return apply_filters( 'tf_template_types', array(
			array( 'name' => __('Archive', 'madx-flow'), 'value' => 'archive' ),
			array( 'name' => __('Single', 'madx-flow'), 'value' => 'single' ),
			array( 'name' => __('Page', 'madx-flow'), 'value' => 'page' )
		));
	}
	...
	public static function module_types() {
		return apply_filters( 'tf_module_types', array(
			array( 'name' => __('Global', 'madx-flow'), 'value' => 'global' ),
			array( 'name' => __('Archive', 'madx-flow'), 'value' => 'archive' ),
			array( 'name' => __('Single', 'madx-flow'), 'value' => 'single' ),
			array( 'name' => __('Page', 'madx-flow'), 'value' => 'page' ),
			array( 'name' => __('Content', 'madx-flow'), 'value' => 'content' ),
		));
	}
	...
	public static function save_styling( $post_id, $data ) {
		...
		if ( ! $global || ! $atomic ) {
			$error = new WP_Error();
			if ( ! $global ) {
				$error->add( 'file_global_write_error', __( 'Global stylesheet could not be created.', 'madx-flow' ) );
			}
			if ( ! $atomic ) {
				$error->add( 'file_atomic_write_error', __( 'Atomic stylesheet could not be created.', 'madx-flow' ) );
			}
			return $error;
		}
	...
	public static function get_grid_settings( $setting = 'grid') {
		...
		$gutters = array(
			array( 'name' => __('Default', 'madx-flow'), 'value' => 'tf_gutter_default' ),
			array( 'name' => __('Narrow', 'madx-flow'), 'value' => 'tf_gutter_narrow' ),
			array( 'name' => __('None', 'madx-flow'), 'value' => 'tf_gutter_none' ),
		);
	...
	public static function get_web_safe_font_list($only_names = false) {
		...
		if( ! $only_names ) {
			$web_safe_fonts = array(
				...
				array('value' => '', 'name' => '--- '.__('Web Safe Fonts', 'madx-flow').' ---')
			);
