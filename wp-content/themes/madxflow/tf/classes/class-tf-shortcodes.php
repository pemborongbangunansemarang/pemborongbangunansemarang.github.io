...
class TF_Shortcodes {
	...
	public static function row( $atts, $content = null ) {
		...
		if ( isset( $atts['editable_markup'] ) && 'true' == $atts['editable_markup'] ) {
			$style_link = sprintf( '<div class="tf_styling_menu tf_row_top_item"><a href="#" title="%s" class="tf_styling_row tf-tooltips"><span class="ti-brush"></span></a></div>', __('Styling', 'madx-flow') );
	...
	public static function column( $atts, $content = null ) {
		...
		$after = '';
		if ( isset( $atts['editable_markup'] ) && 'true' == $atts['editable_markup'] ) {
			$before = sprintf( '<div class="tf_module_holder"><div class="tf_empty_holder_text">%s</div>', __('Drop module here', 'madx-flow' ) );
			$after = '</div>';
	...
	public static function row_fields() {
		return apply_filters( 'tf_row_option_fields', array(
			...
			'row_width' => array(
				'type' => 'radio',
				'label' => __('Row Width', 'madx-flow'),
				'options' => array(
					array( 'name' => __('Default', 'madx-flow'), 'value' => 'tf_row_width_default', 'selected' => true),
					array( 'name' => __('Fullwidth', 'madx-flow'), 'value' => 'tf_row_full_width'),
				)
			),
			'row_height' => array(
				'type' => 'radio',
				'label' => __( 'Row Height', 'madx-flow'),
				'options' => array(
					array( 'name' => __('Default', 'madx-flow'), 'value' => 'tf_row_height_default', 'selected' => true ),
					array( 'name' => __('Fullheight (100% viewport height)', 'madx-flow'), 'value' => 'tf_row_full_height' )
				)
			),
			...
			'class' => array(
				'type' => 'text',
				'label' => __('Additional CSS class', 'madx-flow'),
				'class' => 'tf_input_width_80',
				'description' => '<br/>' . __('Add additional CSS class(es) for custom styling', 'madx-flow')
			),
			'row_anchor' => array(
				'type' => 'text',
				'label' => __('Row Anchor', 'madx-flow'),
				'class' => 'tf_input_width_80',
				'description' => '<br/>' . __('Example: enter &quot;about&quot; as row anchor and add &quot;#about&quot; link in navigation menu. When link is clicked, it will scroll to this row.', 'madx-flow')
			)
		) );
	}
