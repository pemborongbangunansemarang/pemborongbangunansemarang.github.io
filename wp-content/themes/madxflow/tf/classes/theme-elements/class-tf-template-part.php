...
class TF_Template_Part {
	...
	public function __construct(){
		...
		add_action( 'after_setup_theme', array( $this, 'madx_builder_support' ) );
	}
	...
	public function register_post_types() {
		...
		register_post_type( $this->post_type,
			apply_filters( 'tf_register_post_type_tf_template_part', array(
				'labels' => array(
					'name'               => __( 'Template Parts', 'madx-flow' ),
					'singular_name'      => __( 'Template Part', 'madx-flow' ),
					'menu_name'          => _x( 'Template Parts', 'admin menu', 'madx-flow' ),
					'name_admin_bar'     => _x( 'Template Part', 'add new on admin bar', 'madx-flow' ),
					'add_new'            => _x( 'Add New', 'template part', 'madx-flow' ),
					'add_new_item'       => __( 'Add New Template Part', 'madx-flow' ),
					'new_item'           => __( 'New Template Part', 'madx-flow' ),
					'edit_item'          => __( 'Edit Template Part', 'madx-flow' ),
					'view_item'          => __( 'View Template Part', 'madx-flow' ),
					'all_items'          => __( 'All Template Parts', 'madx-flow' ),
					'search_items'       => __( 'Search Template Parts', 'madx-flow' ),
					'parent_item_colon'  => __( 'Parent Template Parts:', 'madx-flow' ),
					'not_found'          => __( 'No template parts found.', 'madx-flow' ),
					'not_found_in_trash' => __( 'No template parts found in Trash.', 'madx-flow' )
				),
				'public'              => false,
	...
	public function render_form() {
		...
		echo TF_Form::submit_button( __('Add', 'madx-flow') );
		echo TF_Form::close();
	}
	...
	public function validate_fields( $fields ) {
		...
		$validator = new TF_Validator( $fields, apply_filters( 'tf_form_validate_template_part_field_rules', array(
			'tf_template_part_name' => array( 'rule' => 'notEmpty', 'error_msg' => __('You should enter the template part name', 'madx-flow') )
		)));
	...
	public function saving_form( $post_data ) {
		global $TF;
		$name = isset( $post_data['tf_template_part_name'] ) ? sanitize_text_field( $post_data['tf_template_part_name'] ) : __('New Template Part', 'madx-flow');
	...
	public function options_metaboxes() {
		add_meta_box(
			...
			__( 'Template Part Options', 'madx-flow' ),
	...
	/**
	 * madx Builder plugin compatibility
	 *
	 * @since 1.1.8
	 */
	function madx_builder_support() {
		if( class_exists( 'madx_Builder' ) ) {
			add_filter( 'madx_builder_post_types_support', array( $this, 'add_madx_builder_support' ) );
			add_filter( 'madx_post_types', array( $this, 'add_madx_builder_support' ) );
	 ...
	 * @since 1.1.8
	 */
	function add_madx_builder_support( $post_types ) {
	...
	function builder_render( $output, $post_id ) {
		...
		$builder_data = $madxBuilder->get_builder_data( $post_id );
		$output .= $madxBuilder->retrieve_template( 'builder-output.php', array( 'builder_output' => $builder_data, 'builder_id' => $post_id ), '', '', false );
