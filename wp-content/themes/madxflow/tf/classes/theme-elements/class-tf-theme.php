...
class TF_Theme {
	...
	public function __construct(){
		$this->defaults = array(
			'tf_theme_name'        => __('New Theme', 'madx-flow'),
	...
	public function register_post_types() {
		...
		register_post_type( $this->post_type,
			apply_filters( 'tf_register_post_type_tf_theme', array(
				'labels' => array(
					'name'               => __( 'Themes', 'madx-flow' ),
					'singular_name'      => __( 'Theme', 'madx-flow' ),
					'menu_name'          => _x( 'Themes', 'admin menu', 'madx-flow' ),
					'name_admin_bar'     => _x( 'Theme', 'add new on admin bar', 'madx-flow' ),
					'add_new'            => _x( 'Add New', 'theme', 'madx-flow' ),
					'add_new_item'       => __( 'Add New Theme', 'madx-flow' ),
					'new_item'           => __( 'New Theme', 'madx-flow' ),
					'edit_item'          => __( 'Edit Theme', 'madx-flow' ),
					'view_item'          => __( 'View Theme', 'madx-flow' ),
					'all_items'          => __( 'All Themes', 'madx-flow' ),
					'search_items'       => __( 'Search Themes', 'madx-flow' ),
					'parent_item_colon'  => __( 'Parent Themes:', 'madx-flow' ),
					'not_found'          => __( 'No themes found.', 'madx-flow' ),
					'not_found_in_trash' => __( 'No themes found in Trash.', 'madx-flow' )
				),
				'public'              => false,
	...
	public function edit_columns( $columns ) {
		$columns = array(
			...
			'title' => __('Theme', 'madx-flow'),
			'screenshot' => __('Screenshot', 'madx-flow'),
			'author' => __('Author', 'madx-flow'),
			'status' => __('Status', 'madx-flow'),
	...
	public function fields() {
		return apply_filters( 'tf_post_type_tf_theme_fields', array(
			...
			'tf_theme_description' => array(
				'type'  => 'textarea',
				'label' => __( 'Description', 'madx-flow' )
			),
			'tf_theme_author'      => array(
				'type'  => 'text',
				'label' => __( 'Author', 'madx-flow' )
			),
			'tf_theme_author_link' => array(
				'type'  => 'text',
				'label' => __( 'Author Link', 'madx-flow' )
			),
			'tf_theme_version'     => array(
				'type'    => 'text',
				'label'   => __( 'Version', 'madx-flow' ),
				'default' => '1.0.0'
			),
			'tf_theme_screenshot'  => array(
				'type'  => 'image',
				'label' => __( 'Screenshot', 'madx-flow' ),
			...
			'tf_import_base_template_and_part' => array(
				'type'    => 'checkbox',
				'label'   => __('Base Templates & Parts', 'madx-flow'),
				'text'    => __('Import the base Templates & Parts', 'madx-flow'),
				'checked' => true
			)
		));
	}
	...
	public function render_form( $method ) {
		$label_submit_btn = 'add' == $method ? __('Add', 'madx-flow') : __('Save', 'madx-flow');
		$data = array();
		$fields = $this->fields();
		if ( 'add' != $method ) {
	...
	public function validate_fields( $fields ) {
		...
		$validator = new TF_Validator( $fields, apply_filters( 'tf_form_validate_theme_field_rules', array(
			'tf_theme_name' => array( 'rule' => 'notEmpty', 'error_msg' => __('You must enter the theme name', 'madx-flow') ),
			'tf_theme_description' => array( 'rule' => 'notEmpty', 'error_msg' => __('You must enter the theme description', 'madx-flow') ),
			'tf_theme_author' => array( 'rule' => 'notEmpty', 'error_msg' => __('You must enter the author name', 'madx-flow') )
		)));
