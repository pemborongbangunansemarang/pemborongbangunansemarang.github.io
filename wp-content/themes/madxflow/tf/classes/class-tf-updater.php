...
class TF_Updater {
	...
	public function init() { 
        ...
        public function add_settings($sections){
            $sections['licenses'] = array('title' => __( 'Licenses', 'madx-flow' ),
       ...
       function theme_update_transient( $value ) {
		...
		if ( false === $update_data ) {
			...
			$response = wp_remote_post(self::madx_STORE_URL, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
	 ...
	 */
        ...
        public function activate_license(){
            ...
            if( isset( $_POST['key'] ) && isset($_POST[$_POST['key']])) {
                    ...
                    $resp['btn'] = $resp['status']=='valid'?__('Deactivate License','madx-flow'):__('Activate License','madx-flow');
	 ...
	 */
        ...
        private function get_error_notifaction($error_type){
            switch($error_type){
                case 'expired': 
                    return __('License key is expired','madx-flow');
                case 'invalid' :
                case 'missing':
                case 'failed':
                    return __('License key is invalid','madx-flow');
                case 'item_name_mismatch':
                    return __('Incorrect Item name','madx-flow');
                case 'revoked':
                    return __('License key is disabled','madx-flow');
                case 'invalid_item_id':
                    return __('Incorrect Item id','madx-flow');
                case 'no_activations_left':
                    return __('This license key is already used','madx-flow');
                default:
                    return '';
            }
                     ...
                     <tbody>
                         <?php foreach($licenses as $key=>$license):?>
                             <tr>
                                 ...
                                 <td>
                                     <input type="text" name="<?php echo $key?>" id="<?php echo $key?>" class="regular-text" placeholder="<?php _e('Enter your license key','madx-flow'); ?>" value="<?php esc_attr_e($license['license'] ); ?>" />                                                
                                     ...
                                     <div class="tf_date_expires">
                                         <?php if((!isset($license['error']) || !$license['error']) && isset($license['expires'])):?>
                                             <?php _e('Date expires','madx-flow')?>: <?php echo $license['expires']?>
                                         <?php endif;?>
                                     </div>
                                 </td>
                                 <td class="tf_auto_update_input">
                                     ...
                                     <label for="<?php echo $key?>_auto"><?php _e('AutoUpdate','madx-flow')?></label>
                                 </td>
                                 <td>
                                     <input type="button" data-key="<?php echo $key?>" class="<?php echo self::PREFFIX?>_activation_btn button-secondary" value="<?php $license['status'] == 'valid'?_e('Deactivate License','madx-flow'):_e('Activate License','madx-flow'); ?>"/>
                                 </td>
                             </tr>
                         <?php endforeach;?>
                     </tbody>
	 ...
	 */
        public function activate_auto(){
            if(isset($_POST['akey']) && $_POST['akey'] && isset($_POST['value'])){
                ...
                // retrieve the license from the database
                if(!isset($licenses_array[$key])){
                    die(json_encode(array('msg'=>__("Cann't find the addon",'madx-flow'))));
                }
                if($licenses_array[$key]['status']!='valid'){
                     die(json_encode(array('msg'=>__("You need to activate license",'madx-flow'))));
                }
                ...
                $msg.= isset($licenses_array[$key]['auto'])?__(' autoupdate is activated','madx-flow'):__(' autoupdate is deactivated','madx-flow');
	...
	private function api_request( $_action, $_data ) {
		...
		if( self::madx_STORE_URL == home_url() ) {
		...
		$request = wp_remote_post( self::madx_STORE_URL, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
