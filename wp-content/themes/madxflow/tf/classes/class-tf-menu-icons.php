...
class TF_Menu_Icons {
	...
	function wp_edit_nav_menu_walker( $walker ) {
		...
		return 'madx_Flow_Walker_Nav_Menu_Edit';
	}
	...
	function wp_nav_menu_item_custom_fields( $item_id, $item, $depth, $args ) {
		$saved_meta = madxflow_get_menu_icon( $item_id );
	?>
		<p class="field-icon description description-thin">
			<label for="edit-menu-item-icon-<?php echo esc_attr( $item_id ); ?>">
				<?php _e( 'Icon', 'madx' ) ?><br/>
				<input type="text" name="menu-item-icon[<?php echo esc_attr( $item_id ); ?>]" id="edit-menu-item-icon-<?php echo esc_attr( $item_id ) ?>" size="8" class="edit-menu-item-icon madx_field_icon" value="<?php echo esc_attr( $saved_meta ); ?>">
				<a class="button button-secondary hide-if-no-js tf_fa_toggle" href="#"><?php _e( 'Insert Icon', 'madx-flow' ) ?> </a>
			</label>
		</p>
	<?php }
	...
	function wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $args ) {
		...
		$meta_value = madxflow_get_menu_icon( $menu_item_db_id );
	...
	function menu_icon( $title, $id = '' ) {
		if ( '' != $id ) {
			if ( $icon = madxflow_get_menu_icon( $id ) ) {
 ...
 * @since 1.0.5
 */
function madxflow_get_menu_icon( $item_id ) {
