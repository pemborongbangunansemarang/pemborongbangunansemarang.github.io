...
class TF_Frontend {
	...
	public function enqueue_scripts() {
		...
		if ( TF_Model::is_tf_editor_active() ) {
			wp_enqueue_style( 'tf-icons', $framework_url . '/assets/css/madx-icons/madx-icons.css', array(), $version );
			...
			wp_enqueue_style( 'tf-minicolors-css', $framework_url . '/assets/css/jquery.minicolors.css', array(), $version );
                     ...
                     wp_enqueue_script('madx-scroll', $framework_url . '/assets/js/madx.scroll-highlight.js' , array( 'jquery' ), $version, true );
                }

		/* FontAwesome library
		 * uses the same handle as used by madx Builder to prevent double loading the fonts */
			...
			foreach( $load_app_scripts as $handle => $script ) {
				...
				if ( 'tf-app-js' == $handle ) {
					...
					wp_localize_script( 'tf-app-js', '_tf_app', array( 
						'post_id' => get_the_ID(),
						'template_part_delete' => __('Are you sure to remove this template part?', 'madx-flow' ),
						...
						'clear_style_text' => __('Do you want to clear all Styling data in this panel?', 'madx-flow'),
						'base_path' => $TF->framework_path(),
						'base_uri' => $framework_url,
						'beforeunload' => __( 'You have unsaved changes. Please save the builder data.', 'madx-flow'),
						'isTouch' => wp_is_mobile(),
						'leaving_template_lightbox' => __('Close editor without saving the changes ?', 'madx-flow')
					) );
	...
	public function validate_region_fields( $fields ) {
		...
		$validator = new TF_Validator( $fields, apply_filters( 'tf_builder_form_validate_region_template_part_field_rules', array(
			'tf_template_part_shortcode' => array( 'rule' => 'notEmpty', 'error_msg' => __('You should select template part', 'madx-flow') )
		)));
	...
	public function saving_region_form( $post_data ) {
		...
		$data = array(
			...
			'edit_url'  => $edit_url,
			'slug'      => $slug,
			'caption'   => sprintf( __('Template Part: %s', 'madx-flow'), TF_Model::get_template_part_title( $slug ) ),
			'styles'	=> $css,
		);
	...
	public function frontend_menu() {
		...
		$menus_list = array(
			...
			'template_option' => array(
				'label' => __('Template Options', 'madx-flow'),
				'href' => '#',
		...
		if ( is_singular( 'tf_template' ) && ! TF_Model::is_tf_editor_active() ) {
			...
			$menus['edit_template_part'] = array(
				'label' => __('Edit Template Part', 'madx-flow'),
				'href' => esc_url( add_query_arg( 'tf', 1, get_permalink( get_the_ID() ) ) )
			);
		} else if ( is_singular( 'tf_template_part' ) && TF_Model::is_tf_editor_active() ) {
			...
			if ( 0 !== $TF_Layout->layout_id ) {
				$menus['in_use_template'] = array(
					'label' => __('Edit In-use Template', 'madx-flow'),
			...
			$menus['template_part_list'] = array(
				'label' => __('Template Parts', 'madx-flow'),
				'href' => '#'
			);
			...
			$menus['template_part_list_view'] = array(
				'label' => __('View all Template Parts', 'madx-flow'),
			...
			if ( TF_Model::is_tf_styling_active() ) {
				$menus['global_styling'] = array(
					'label' => __('Global Styling', 'madx-flow'),
					'href'  => '#',
