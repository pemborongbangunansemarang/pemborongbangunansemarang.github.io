...
class TF_Settings {
	 ...
	 * @var string $slug
	 */
	static protected $slug = 'madx-flow';
	...
	public static function get_sections_and_settings() {
		$sections['general'] = array(
			'title' => __( 'General', 'madx-flow' ),
			'callback' => array( 'TF_Settings', 'general_description' ),
			'fields' => array(
				array(
					'id' => 'favicon',
					'label' => __( 'Favicon', 'madx-flow'),
					'type' => 'text',
					'class' => 'large-text',
					'default' => '',
					'description' =>'',
				...
				array(
					'id' => 'header_text',
					'label' => __( 'Header Code', 'madx-flow' ),
					'type' => 'textarea',
					'class' => 'large-text',
					'options' => '15',
					'description' => __('The code will be added to the <head> tag (use it to add additional scripts such as CSS or JS).', 'madx-flow'),
					'default' => ''
				),
				array(
					'id' => 'footer_text',
					'label' => __( 'Footer Code', 'madx-flow' ),
					'type' => 'textarea',
					'class' => 'large-text',
					'options' => '15',
					'description' => __('The code will be added to the footer before the closing </body> tag (use it to add JavaScript, Google Analytics code, etc).', 'madx-flow'),
					'default' => ''
				),
				array(
					'id' => 'disable_responsive',
					'label' => __( 'Responsive Design', 'madx-flow' ),
					'type' => 'checkbox',
					'default' => '',
					'description' => __( 'Check to disable responsive design', 'madx-flow' )
				),
				array(
					'id'   => 'webfonts',
					'label'=> __( 'Google Fonts', 'madx-flow' ),
					'type' => 'google_fonts',
					'description' => __('Enter the additional character subsets you need to use from Google Fonts separated by commas. Example: latin-ext,cyrillic.', 'madx-flow'),
					'default' => ''
				)
			),
		);
    ...
    public static function google_fonts( $args ) {
    	...
    	<p>
    		...
    		<label for="<?php echo esc_attr( self::$slug . '-'.$field_id.'-list-recommended' ); ?>"><?php _e('Show recommended Google Fonts only', 'madx-flow')?></label>
    	</p>
    	<p>
    		...
    		<label for="<?php echo esc_attr( self::$slug . '-'.$field_id.'-list-full' ); ?>"><?php _e('Show all Google Fonts (showing all fonts will take longer to load)', 'madx-flow')?></label>
    	</p>
    	<br/>
    	<p>
    		...
    		<label for="<?php echo esc_attr( self::$slug . '-'.$field_id.'-subsets' ); ?>"><?php _e('Character Subsets', 'madx-flow')?></label>
    	</p>
    	<?php if ( isset( $args['description'] ) ) : ?>
    		<span class='howto'><?php echo wp_kses_post( $args['description'] ); ?></span>
    	<?php endif;?>
