...
final class TF_Form {
	...
	static public function render( $fields, $exist_data = array() ) {
		...
		foreach( $fields as $key => $field ) {
			...
			if ( $field['type'] == 'multi' ) {
				...
				$output .= '</ul>';
				$output .= '<a class="tf_add_row">' . ( isset( $field['new_row_text'] ) ? $field['new_row_text'] : __( 'Add New Row', 'madx-flow' ) ) . '</a>';
			} else {
				$output .= self::print_field( $key, $field, $exist_data );
			}

			$output .= '<span class="error"></span>';
			$output .= $field['type'] != 'separator' && 'yes' == $field['wrapper'] ? '</div>' : ''; // madx_builder_input
			$output .= $field['type'] != 'separator' ? '</div>' : ''; // madx_builder_field	
		}
		return $output;
	}
	...
	static public function render_element( $fields, $exist_data = array() ) {
		...
		foreach( $fields as $key => $field ) {
			...
			$output .= '<span class="error"></span>';
			$output .= 'yes' == $field['wrapper'] ? '</div>' : ''; // madx_builder_input
			$output .= $field['type'] != 'separator' ? '</div>' : ''; // madx_builder_field	
		}
		return $output;
	}
		...
		switch ( $field['type'] ) {
			...
			case 'video': 
				...
				if ( is_multisite() && !is_upload_space_available() ) {
					$output .= sprintf( __( '<p>Sorry, you have filled your %s MB storage quota so uploading has been disabled.</p>', 'madx-flow' ), get_space_allowed() );
				} else {
					$output .= sprintf( '<a href="#" class="tf_upload_media_library" data-uploader-title="%s" data-uploader-button-text="%s" data-library-type="%s">%s</a>',
							'image' == $field['type'] ? esc_attr__('Upload an Image', 'madx-flow') : esc_attr__('Upload a Video', 'madx-flow'),
							esc_attr__('Insert file URL', 'madx-flow'),
							esc_attr( $field['type'] ),
							'image' == $field['type'] ? esc_attr__( '+ Add Image', 'madx-flow') : esc_attr__( '+ Add Video', 'madx-flow') 
					);
					
					$max_upload_size = (int) wp_max_upload_size() / ( 1024 * 1024 );
					$output .= sprintf( __( '<p class="tf_max_upload_size">Maximum upload file size: %d MB.</p>', 'madx-flow' ), $max_upload_size );
				}
				$output .= '</div>';
				if ( 'image' == $field['type'] ) {
			...
			case 'template_part_select':
				...
				if ( ( isset( $field['show_extra_link'] ) && true == $field['show_extra_link'] ) || ! isset( $field['show_extra_link'] ) ) {
					$output .= sprintf( '<a href="%s" target="_blank" class="manage_template_part_link"><span class="tf_icon ti-folder"></span> %s</a>',
						...
						__( 'Manage Template Part', 'madx-flow' )
					);
				}
			break;
			...
			case 'icon':
				...
				$output .= '<a class="button button-secondary hide-if-no-js tf_fa_toggle" href="#">' . __( 'Insert Icon', 'madx-flow' ) .'</a>';
			break;
			...
			case 'gallery':
				...
				$output .= sprintf('<a href="#" class="tf_browse_gallery_btn">%s</a>', __('+ Insert Gallery', 'madx-flow'));
	...
	static public function submit_button( $text, $name = 'submit' ) {
		return sprintf('<p class="tf_lightbox_save">
				...
				</p>', __('Cancel', 'madx-flow'), esc_attr( $text ), esc_attr( $name ) );
	}
	...
	static private function print_archive_tabs( $field_name, $selected = array() ) {
		...
		$output .= '<li><a href="#visibility-tab-archive-archives">' . __( 'Archives', 'madx-flow' ) . '</a></li>';
		$output .= '<li><a href="#visibility-tab-archive-taxonomies">' . __( 'Taxonomies', 'madx-flow' ) . '</a></li>';
		$output .= '</ul>';

		foreach( $taxonomies as $key => $tax ) {
			$output .= '<div id="visibility-tab-archive-'. $key .'" class="madx-visibility-options clearfix">';
			$categories = get_terms( $key, array( 'hide_empty' => true ) );

			$output .= wp_kses_post( sprintf( __( '<p><small>Check which %s where the template will be used for the %s views.</small></p>', 'madx-flow' ), $tax->labels->name, $tax->labels->singular_name ) );

			$checked = isset( $selected['archive'][ $key ]['all'] ) ? checked( $selected['archive'][ $key ]['all'], 'on', false ) : '';
			$output .= '<label class="label-full"><input class="tf_toggle_prop" type="checkbox" name="'.$field_name.'[archive]['.$key.'][all]" '. $checked .' />' . __('Apply to all', 'madx-flow') . '</label>';
		...
		// Archives tab
		$output .= '<div id="visibility-tab-archive-archives" class="madx-visibility-options clearfix">';
		$archives = array(
			'is_search' => __('Search', 'madx-flow'),
			'is_date' => __('Date', 'madx-flow'),
			'is_author' => __('Author', 'madx-flow'),
			'is_year' => __('Year', 'madx-flow'),
			'is_day' => __('Day', 'madx-flow'),
			'is_month' => __('Month', 'madx-flow'),
			'is_home' => __('Latest Posts Homepage', 'madx-flow'),
		);

		$output .= wp_kses_post( __( '<p><small>Check which archive views where the template will be used. Note: if "Apply to all" is checked, it will apply to the archive views of all other custom post types as well.</small></p>', 'madx-flow' ) );

		$checked = isset( $selected['archive']['archive']['all'] ) ? checked( $selected['archive']['archive']['all'], 'on', false ) : '';
		$output .= '<label class="label-full"><input class="tf_toggle_prop" type="checkbox" name="'.$field_name.'[archive][archive][all]" '. $checked .' />' . __('Apply to all', 'madx-flow') . '</label>';
		...
		// Taxonomies tab
		$output .= '<div id="visibility-tab-archive-taxonomies" class="madx-visibility-options clearfix">';
		$taxonomies = TF_Model::get_taxonomies( array(), array( 'category', 'post_tag' ) );

		$output .= wp_kses_post( __( '<p><small>Check which taxonomy archive views where the template will be used. Note: if "Apply to all" is checked, it will apply to the archive views of all other custom taxonomies as well.</small></p>', 'madx-flow' ) );

		$checked = isset( $selected['archive']['tax']['all'] ) ? checked( $selected['archive']['tax']['all'], 'on', false ) : '';
		$output .= '<label class="label-full"><input class="tf_toggle_prop" type="checkbox" name="'.$field_name.'[archive][tax][all]" '. $checked .' />' . __('Apply to all', 'madx-flow') . '</label>';
	...
	static private function print_single_tabs( $field_name, $selected = array() ) {
		...
		/* build the tab links */
		$output .= '<li><a href="#visibility-tab-single-category">' . __( 'Categories', 'madx-flow' ) . '</a></li>';
		$output .= '<li><a href="#visibility-tab-single-post-types">' . __( 'Post Types', 'madx-flow' ) . '</a></li>';
		$output .= '<li><a href="#visibility-tab-single-singles">' . __( 'Posts', 'madx-flow' ) . '</a></li>';
		$output .= '</ul>';

		// Categories Tab
		$output .= '<div id="visibility-tab-single-category" class="madx-visibility-options clearfix">';
		$categories = get_terms( 'category', array( 'hide_empty' => true ) );

		$output .= wp_kses_post( __( '<p><small>Check which categories to apply this template to the single view of each entry filed under the category.</small></p>', 'madx-flow' ) );

		$checked = isset( $selected['single']['category']['all'] ) ? checked( $selected['single']['category']['all'], 'on', false ) : '';
		$output .= '<label class="label-full"><input class="tf_toggle_prop" type="checkbox" name="'.$field_name.'[single][category][all]" '. $checked .' />' . __('Apply to all', 'madx-flow') . '</label>';
		...
		// Post Types tab
		$output .= '<div id="visibility-tab-single-post-types" class="madx-visibility-options clearfix">';
		$post_types = TF_Model::get_post_types( array(), array('tf_template', 'tf_template_part' ) );

		$output .= wp_kses_post( __( '<p><small>Check which post types to apply this template to the single view of each entry of their type. Note: if "Apply to all" is checked, it will apply to all single views of all other custom post types as well.</small></p>', 'madx-flow' ) );

		$checked = isset( $selected['single']['post_type']['all'] ) ? checked( $selected['single']['post_type']['all'], 'on', false ) : '';
		$output .= '<label class="label-full"><input class="tf_toggle_prop" type="checkbox" name="'.$field_name.'[single][post_type][all]" '. $checked .' />' . __('Apply to all', 'madx-flow') . '</label>';
		...
		// Posts tab
		$output .= '<div id="visibility-tab-single-singles" class="madx-visibility-options clearfix">';
		$query_posts = get_posts( array( 
			'post_type' => array_keys( TF_Model::get_post_types( array(), array( 'page', 'tf_template', 'tf_template_part' ) ) ),
			'posts_per_page' => -1 
		) );

		$output .= wp_kses_post( __( '<p><small>Check which posts to apply this template to its single view.</small></p>', 'madx-flow' ) );

		$checked = isset( $selected['single']['singular']['all'] ) ? checked( $selected['single']['singular']['all'], 'on', false ) : '';
		$output .= '<label class="label-full"><input class="tf_toggle_prop" type="checkbox" name="'.$field_name.'[single][singular][all]" '. $checked .' />' . __('Apply to all', 'madx-flow') . '</label>';
	...
	static private function print_page_tabs( $field_name, $selected = array() ) {
		...
		// Page Tab
		$output .= '<div id="visibility-tab-page-page" class="madx-visibility-options ui-tabs-panel clearfix">';
		$key = 'page';
		$pages = get_posts( array( 'post_type' => $key, 'posts_per_page' => -1, 'status' => 'published' ) );

		$output .= wp_kses_post( __( '<p><small>Check which pages to apply this template to.</small></p>', 'madx-flow' ) );

		$checked = isset( $selected['page']['all'] ) ? checked( $selected['page']['all'], 'on', false ) : '';
		$output .= '<label class="label-full"><input class="tf_toggle_prop" type="checkbox" name="'.$field_name.'[page][all]" '. $checked .' />' . __('Apply to all', 'madx-flow') . '</label>';
		$checked = isset( $selected['page']['404'] ) ? checked( $selected['page']['404'], 'on', false ) : '';
		$output .= '<label><input type="checkbox" name="'.$field_name.'[page][404]" '. $checked .' />' . __('404', 'madx-flow') . '</label>';
		$checked = isset( $selected['page']['is_front_page'] ) ? checked( $selected['page']['is_front_page'], 'on', false ) : '';
		$output .= '<label><input type="checkbox" name="'.$field_name.'[page][is_front_page]" '. $checked .' />' . __('Static Front Page', 'madx-flow') . '</label>';
