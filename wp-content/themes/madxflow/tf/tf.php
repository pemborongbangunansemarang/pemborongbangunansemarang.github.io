...
final class madx_Flow {
	...
	public function setup_framework() {
		...
		// Load theme domain
		load_theme_textdomain( 'madx-flow', $this->framework_path() . '/languages' );
	...
	/**
	 * Remove last part of title to keep the traditional madx theme title.
	 *
	 * @since 1.0.9
	 *
	 * @param string $title
	...
	private function check_update() {
		if ( is_admin() && current_user_can( 'manage_options' ) ) {
			...
			$madx_builder_updater = new TF_Update_Check( array(
				'name' 		  => 'madxflow',
				'nicename' 	  => 'madx Flow',
				'update_type' => 'theme',
			), $this->version );
	...
	public function get_plupload_settings() {
		return apply_filters('tf_plupload_init_vars', array(
			...
			'filters' 				=> array( array(
				'title' => __('Allowed Files', 'madx-flow'),
