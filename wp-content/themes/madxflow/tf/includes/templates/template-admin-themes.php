...
wp_localize_script( 'tf-theme-js', '_tfThemeSettings', array(
	...
	'settings' => array(
 	...
 	'l10n' => array(
 		'addNew'            => __( 'Add New Theme', 'madx-flow' ),
 		'search'            => __( 'Search Installed Themes', 'madx-flow' ),
 		'searchPlaceholder' => __( 'Search installed themes...', 'madx-flow' ), // placeholder (no ellipsis)
		'themesFound'       => __( 'Number of Themes found: %d', 'madx-flow' ),
		'noThemesFound'     => __( 'No themes found. Try a different search.', 'madx-flow' ),
  	),
) );
?>
...
<div class="wrap">
	...
	<h2><?php _e('Flow Themes', 'madx-flow') ?>
		<span class="title-count theme-count"><?php echo count( $themes ); ?></span>
		<a data-type="theme" class="add-new-h2 tf_lightbox_new" href="#"><?php _e('Add New', 'madx-flow') ?></a>
		<a data-type="theme" class="add-new-h2 tf_lightbox_import" href="#"><?php _e('Add via Import', 'madx-flow') ?></a>
	</h2>
	...
	<div class="theme-browser tf-theme-browser">
		<div class="themes tf-themes">
			...
			<div class="theme tf-theme<?php if ( $theme['active'] ) echo ' active'; ?>" tabindex="0" aria-describedby="<?php echo $aria_action . ' ' . $aria_name; ?>">
				...
				<?php if ( $theme['active'] ) { ?>
					<a class="button button-primary tf_lightbox_edit" href="#" data-type="theme" data-post-id="<?php echo $theme['theme_id']; ?>"><?php _e( 'Edit', 'madx-flow' ); ?></a>
				<?php } else { ?>
					<a class="button button-secondary" href="<?php echo $theme['actions']['activate']; ?>"><?php _e( 'Activate', 'madx-flow' ); ?></a>
					<a class="button button-primary tf_lightbox_edit" href="#" data-type="theme" data-post-id="<?php echo $theme['theme_id']; ?>"><?php _e( 'Edit', 'madx-flow' ); ?></a>
				<?php } ?>
...
<script id="tmpl-tf-theme" type="text/template">
	...
	<# if ( data.active ) { #>
		<a class="button button-primary tf_lightbox_edit" href="#" data-type="theme" data-post-id="{{{ data.theme_id }}}"><?php _e( 'Edit', 'madx-flow' ); ?></a>
	<# } else { #>
		...
		<div class="theme-actions">
			<div class="active-theme">
				...
				<# if ( data.actions.export ) { #>
					<a href="{{{ data.actions.export }}}" class="button button-secondary"><?php _e( 'Export', 'madx-flow' ); ?></a>
				<# } #>
				<a class="button button-secondary tf_lightbox_duplicate" href="#" data-type="theme" data-post-id="{{{ data.theme_id }}}"><?php _e( 'Duplicate', 'madx-flow' ); ?></a>
				<a class="button button-secondary tf_lightbox_replace" href="#" data-type="theme" data-post-id="{{{ data.theme_id }}}"><?php _e( 'Replace', 'madx-flow' ); ?></a>
			</div>
