...
<div class="tf_interface tf_admin clearfix">
			...
			<div id="tf_module_tabs_<?php echo $type['value']; ?>">
				...
				if ( isset( $module_groups[ $type['value'] ] ) ) {
					...
					<div class="<?php echo esc_attr($class); ?>" data-module-title="<?php echo esc_attr( $module->name ); ?>" data-module-name="<?php echo esc_attr( $module->slug ); ?>">
						...
						<a href="#" title="<?php _e('Add module', 'madx-flow') ?>" class="add_module" data-module-name="<?php echo esc_attr( $module->slug ); ?>"><?php _e('Add', 'madx-flow') ?></a>
					</div>
					<!-- /module -->
					<?php endforeach; ?>
				<?php } //endif ?>
	...
	<div class="tf_row_panel clearfix">
		...
		<div id="tf_row_wrapper" class="tf_content_builder">
			...
			?>
		</div> <!-- /#madx_builder_row_wrapper -->

		<p class="tf_save">
			<?php if ( $pagenow !== 'post-new.php' ): ?>
			<a href="<?php echo esc_url( add_query_arg( 'tf', 1, get_permalink( $post->ID ) ) ); ?>" class="tf_switch_frontend"><?php _e('Switch to frontend', 'madx-flow') ?></a>
			<?php endif; ?>
			<a href="#" id="tf_main_save" class="builder_button tf_btn"><?php _e('Save', 'madx-flow') ?></a>
		</p>

	</div>
	<!-- /madx_builder_row_panel -->
