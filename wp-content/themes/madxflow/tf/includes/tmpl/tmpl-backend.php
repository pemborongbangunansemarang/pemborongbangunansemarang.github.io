<script type="text/html" id="tmpl-tf_import_link">
	<a href="#" class="add-new-h2 tf_lightbox_new" data-type="{{ data.type }}"><?php _e('Add New', 'madx-flow') ?></a>
	<a href="#" class="add-new-h2 tf_lightbox_import" data-type="{{ data.type }}"><?php _e('Add via Import', 'madx-flow') ?></a>
</script>

<script type="text/html" id="tmpl-tf_import_form">
	<?php
	$output = '<div class="tf_import_form_wrapper">'; 
	$output .= sprintf( '<h3>%s</h3>', __( 'Select a file to import', 'madx-flow') );
	...
	if ( is_multisite() && !is_upload_space_available() ) {
		...
		$output .= sprintf( '<p>
				...
				'tf_import_file', 
				'tf_import_file', __('Upload', 'madx-flow') 
		);
		
		$max_upload_size = (int) wp_max_upload_size() / ( 1024 * 1024 );
		$output .= sprintf( __( '<p>Maximum upload file size: %d MB.</p>', 'madx-flow' ), $max_upload_size );
	}
	$output .= '</div>';
	echo $output;
	?>
			...
			<ul class="tf_active_block_menu_ul">
				<li class="tf_active_block_menu_li"><span class="ti-menu"></span>
					<ul>
						...
						<li>
							<a class="tf_lightbox_link_region-edit" data-edit-url="{{ data.edit_url }}" title="<?php _e('Edit', 'madx-flow' ) ?>" href="#">
	...
	<div class="tf_module active_module" data-tf-module-title="{{ data.tf_module_title }}" data-tf-module="{{ data.tf_module }}" data-tf-content="{{ data.content }}" data-tf-atts="{{ data.atts }}">
		<div class="tf_active_module_menu">
			...
			<ul class="tf_dropdown">
				<li>
					<a class="tf_lightbox_link_module-edit" title="<?php _e('Edit', 'madx-flow') ?>" href="#">
						<span class="ti-pencil"></span> <?php _e('Edit', 'madx-flow') ?>
					</a>
				</li>
				<li>
					<a class="tf_lightbox_link_module-duplicate" title="<?php _e('Duplicate', 'madx-flow') ?>" href="#">
						<span class="ti-layers"></span> <?php _e('Duplicate', 'madx-flow') ?>
					</a>
				</li>
				<li>
					<a class="tf_lightbox_link_module-delete" title="<?php _e('Delete', 'madx-flow') ?>" href="#">
						<span class="ti-close"></span> <?php _e('Delete', 'madx-flow') ?>
					</a>
				</li>
			</ul>
		</div>
			...
			<ul class="tf_active_block_menu_ul">
				<li class="tf_active_block_menu_li"><span class="ti-menu"></span>
					<ul>
						...
						<li>
							<a class="tf_lightbox_link_module-style" title="<?php _e('Style', 'madx-flow') ?>" href="#">
	...
	<div class="tf_back_row clearfix tf_gutter_default" data-tf-shortcode="tf_back_row" data-tf-atts="">
		...
		<div class="tf_back_row_content">
			...
			<div class="tf_back_col tf_colfullwidth first" data-tf-shortcode="tf_back_column" data-tf-atts="<?php echo esc_attr( json_encode(array('grid' => 'fullwidth')) ); ?>">
				<div class="tf_module_holder">
					<div class="tf_empty_holder_text"><?php _e('drop module here', 'madx-flow') ?></div>
				</div>
			</div>
			<!-- /tf_back_col -->
...
<script type="text/html" id="tmpl-tf_module_helper">
	<div class="tf_interface tf_module_interface">
		<div class="tf_module module-type-{{ data.name }}" data-module-title="{{ data.title }}" data-module-name="{{ data.name }}">
			...
			<a href="#" title="<?php _e('Add module', 'madx-flow') ?>" class="add_module" data-module-name="{{ data.name }}"><?php _e('Add', 'madx-flow') ?></a>
		</div>
	</div>
</script>
