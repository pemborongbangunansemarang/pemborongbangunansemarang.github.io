...
class TF_Module_Category_Description extends TF_Module {
	...
	public function __construct() {
		parent::__construct( array(
			'name' => __( 'Category Description', 'madx-flow' ),
			'slug' => 'category-description',
			'shortcode' => 'tf_category_description',
			'description' => __( 'Category Description', 'madx-flow' ),
			'category' => 'archive'
		) );
	}
	...
	public function fields() {
		return apply_filters( 'tf_module_category_description_fields', array(
			'first_page_only' => array(
				'label' => __( 'First Page Only', 'madx-flow' ),
				'type' => 'radio',
				'options' => array(
					array( 'name' => __( 'Yes', 'madx-flow' ), 'value' => 'yes', 'selected' => true ),
					array( 'name' => __( 'No', 'madx-flow' ), 'value' => 'no' ),
				),
				'description' => '<br>' . __( 'Display category description on the first category page only', 'madx-flow' )
			)
		) );
	}
	...
	public function styles() {
		return apply_filters( 'tf_module_category_description_styles', array(
			'tf_module_category_description' => array(
				'label' => __( 'Category Description', 'madx-flow' ),
			...
			'first_page_only' => 'yes'
                ...
                if ( ! TF_Model::is_template_page() ) {
                    ...
                    $output = ob_get_clean();
                     ...
                     $output = sprintf('<p>%s</p>', __('<strong>This is only preview text.</strong> The text content here will be replaced with actual category description when viewing the real page.', 'madx-flow') );
                }
		return $output;
	}
}
