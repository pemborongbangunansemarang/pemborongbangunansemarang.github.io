...
class TF_Module_Menu extends TF_module {
	...
	public function __construct() {
		parent::__construct( array(
			'name' => __( 'Menu', 'madx-flow' ),
			'slug' => 'menu',
			'description' => __( 'Menu', 'madx-flow' ),
			'shortcode' => 'tf_menu',
	...
	public function fields() {
		$menu_option = array(
			array( 'value' => '__default', 'name' => __( 'Default Menu', 'madx-flow' ) )
		);
		...
		return apply_filters( 'tf_module_menu_fields', array(
			'nav_menu' => array(
				'type' => 'select',
				'label' => __( 'Select Menu', 'madx-flow' ),
				'options' => $menu_option,
				'description' => '<br/>' . sprintf( __( 'Add menus in <a href="%s">WP > Appearance > Menus</a>', 'madx-flow' ), admin_url( 'nav-menus.php' ) )
			),
			'mobile_menu' => array(
				'label' => __( 'Mobile Menu', 'madx-flow' ),
				'type' => 'radio',
				'options' => array(
			...
			'mobile_menu_breakpoint' => array(
				'type' => 'number',
				'label' => __( 'Trigger Point', 'madx-flow' ),
				'class' => 'tf_input_width_20',
				'default' => 600,
				'description' => 'px<br/>' . __( 'Trigger mobile menu at certain breakpoint (viewport width)', 'madx-flow' ),
				'row_class' => 'mobile_menu mobile_menu-yes',
			),
			'mobile_menu_label' => array(
				'type' => 'text',
				'label' => __( 'Mobile Menu label', 'madx-flow' ),
				'class' => 'tf_input_width_30',
				'default' => __( 'Menu', 'madx-flow' ),
				'row_class' => 'mobile_menu mobile_menu-yes',
			),
		) );
	}
	...
	public function styles() {
		return apply_filters( 'tf_module_menu_styles', array(
			...
			'tf_module_menu_dropdown_link_hover' => array(
				'label' => __( 'Dropdown Link Hover', 'madx-flow' ),
			...
			'tf_module_menu_mobile_menu_container' => array(
				'label' => __( 'Mobile Menu Container', 'madx-flow' ),
			...
			'tf_module_menu_mobile_menu_icon' => array(
				'label' => __( 'Mobile Menu Icon', 'madx-flow' ),
	...
	public function render_shortcode( $atts, $content = null ) {
		extract( shortcode_atts( array(
			...
			'mobile_menu_label' => __( 'Menu', 'madx-flow' ),
		), $atts, $this->shortcode ) );
		...
		if( wp_is_mobile() ) {
			wp_enqueue_script( 'madx-dropdown' );
			$this->footer_scripts .= sprintf( 'jQuery(function(){ jQuery( ".tf_module_block_%s.tf_module_menu .tf_menu" ).madxDropdown() });', $atts['sc_id'] );
		}
	...
	public function register_default_menu_location() {
		register_nav_menus( array(
			'default_menu' => __( 'Default Menu', 'madx-flow' ),
		) );
	}

	public function wp_enqueue_scripts() {
		global $TF;
		wp_register_script( 'madx-dropdown', $TF->framework_uri() . '/assets/js/madx.dropdown.js', array( 'jquery' ), $TF->get_version(), true );
	}
