...
class TF_Element_Post_Excerpt_Content extends TF_Module_Element {
	...
	public function __construct() {
		parent::__construct(array(
			'name' => __('Excerpt/Content', 'madx-flow'),
	...
	public function fields() {
		return apply_filters( 'tf_element_post_excerpt_fields', array(
			'post_display' => array(
				'type' => 'radio',
				'label' => __( 'Excerpt/Content', 'madx-flow'),
				'options' => array(
					array( 'name' => __('Content', 'madx-flow'), 'value' => 'content', 'selected' => true ),
					array( 'name' => __('Excerpt', 'madx-flow'), 'value' => 'excerpt' )
				)
			),
                        'display_inline_block'=> array(
                                'type' => 'checkbox',
                                'label' => __( 'Display Inline', 'madx-flow' ),
                                'text' => __( 'Display this module inline (float left)', 'madx-flow' )
	...
	public function render_shortcode( $atts, $content = null ) {
		...
		if ( 'content' == $atts['post_display'] ) {
			if ( is_object( $post ) ) {
				the_content( __('More &rarr;', 'madx-flow') );
				edit_post_link(__('Edit', 'madx-flow'), '<span class="edit-button">[', ']</span>');
			}
		} else {
			if ( is_object( $post ) ) {
				the_excerpt();
