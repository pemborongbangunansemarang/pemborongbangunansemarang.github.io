...
class TF_Module_Page_Content extends TF_Module {
	...
	public function __construct() {
		parent::__construct( array(
			'name' => __( 'Page Content', 'madx-flow' ),
			'slug' => 'page-content',
			'shortcode' => 'tf_page_content',
			'description' => __( 'Display the content of the page.', 'madx-flow' ),
			'category' => 'page'
		) );
	}
	...
	public function styles() {
		return apply_filters( 'tf_module_page_content_styles', array(
			'tf_module_page_content_container' => array(
				'label' => __( 'Page Content Container', 'madx-flow' ),
	...
	public function render_shortcode( $atts, $content = null ) {
		...
		if ( ! TF_Model::is_template_page() ) {
			...
			$output = sprintf('<p>%s</p>', __('<strong>This is only preview text.</strong> The text content here will be replaced with actual page content when viewing the real page.', 'madx-flow') );
		}
