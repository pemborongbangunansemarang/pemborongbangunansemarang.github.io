...
/**
 * madxartwork Log reporter.
 *
 * madxartwork log reporter handler class is responsible for generating the
 * debug reports.
 *
 * @since 2.4.0
 */
class Log_Reporter extends Base_Reporter {
	...
	const CLEAR_LOG_ACTION = 'madxartwork-clear-log';
	...
	public function get_title() {
		...
		if ( 'html' === $this->_properties['format'] && empty( $_GET[ self::CLEAR_LOG_ACTION ] ) ) { // phpcs:ignore -- nonce validation is not require here.
			...
			$title .= '<a href="' . $url . '#madxartwork-clear-log" class="box-title-tool">' . __( 'Clear Log', 'madxartwork' ) . '</a>';
			$title .= '<span id="madxartwork-clear-log"></span>';
		}
	...
	public function get_log_entries() {
		/** @var \madxartwork\Core\Logger\Manager $manager */
		$manager = Manager::instance();

		/** @var \madxartwork\Core\Logger\Loggers\Db $logger */
		...
		if ( ! empty( $log_entries ) ) {
			...
			foreach ( $log_entries as $key => $log_entry ) {
				if ( $log_entry['count'] ) {
					$entries_string .= '<table><thead><th>' . sprintf( '%s: showing %s of %s', $key, $log_entry['count'], $log_entry['total_count'] ) . '</th></thead><tbody class="madxartwork-log-entries">' . $log_entry['entries'] . '</tbody></table>';
				}
			}
