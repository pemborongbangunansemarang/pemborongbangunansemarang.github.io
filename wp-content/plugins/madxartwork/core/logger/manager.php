<?php
namespace madxartwork\Core\Logger;

use madxartwork\Core\Base\Module as BaseModule;
use madxartwork\Core\Common\Modules\Ajax\Module;
use madxartwork\Core\Logger\Loggers\Logger_Interface;
use madxartwork\Core\Logger\Items\PHP;
use madxartwork\Core\Logger\Items\JS;
...
class Manager extends BaseModule {
	...
	public function shutdown() {
		...
		// `untrailingslashit` in order to include other plugins prefixed with madxartwork.
		$madxartwork_path = untrailingslashit( wp_normalize_path( madxartwork_PATH ) );

		if ( false === strpos( $error_path, $madxartwork_path ) ) {
			return;
		}
	 ...
	 * Javascript log.
	 *
	 * Log madxartwork errors and save them in the database.
	 *
	 * Fired by `wp_ajax_madxartwork_js_log` action.
	 *
	 */
	...
	public function register_default_loggers() {
		$this->register_logger( 'db', 'madxartwork\Core\Logger\Loggers\Db' );
	...
	private function register_loggers() {
		if ( ! did_action( 'madxartwork/loggers/register' ) ) {
			do_action( 'madxartwork/loggers/register', $this );
		}
	}
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_madxartwork_js_log', [ $this, 'js_log' ] );

		add_action( 'madxartwork/loggers/register', [ $this, 'register_default_loggers' ] );
	}
}
