...
use madxartwork\Core\Common\Modules\Connect\Admin;
...
abstract class Base_App {
	...
	const OPTION_NAME_PREFIX = 'madxartwork_connect_';

	const SITE_URL = 'https://my.madxartwork.com/connect/v1';

	const API_URL = 'https://my.madxartwork.com/api/connect/v1';
	...
	public function action_get_token() {
		...
		$this->add_notice( __( 'Connected Successfully.', 'madxartwork' ) );
	...
	public function action_disconnect() {
		if ( $this->is_connected() ) {
			$this->disconnect();
			$this->add_notice( __( 'Disconnected Successfully.', 'madxartwork' ) );
		}
	...
	protected function request( $action, $request_body = [] ) {
		...
		if ( $this->is_connected() ) {
			$headers['X-madxartwork-Signature'] = hash_hmac( 'sha256', wp_json_encode( $request_body, JSON_NUMERIC_CHECK ), $this->get( 'access_token_secret' ) );
		}
	...
	protected function print_popup_close_script( $url ) {
		...
		<script>
			if ( opener && opener !== window ) {
				opener.jQuery( 'body' ).trigger( 'madxartworkConnected' );
				window.close();
				opener.focus();
			} else {
	...
	protected function get_site_key() {
		$site_key = get_option( 'madxartwork_connect_site_key' );

		if ( ! $site_key ) {
			$site_key = md5( uniqid( wp_generate_password() ) );
			update_option( 'madxartwork_connect_site_key', $site_key );
		}
