...
/**
 * madxartwork usage module.
 *
 * madxartwork usage module handler class is responsible for registering and
 * managing madxartwork usage data.
 *
 */
class Module extends BaseModule {
	const GENERAL_TAB = 'general';
	const META_KEY = '_madxartwork_controls_usage';
	const OPTION_NAME = 'madxartwork_controls_usage';
	 ...
	 *
	 * Called on madxartwork/document/before_save, remove document from global & set saving flag.
	 *
	 ...
	 * After document save.
	 *
	 * Called on madxartwork/document/after_save, adds document to global & clear saving flag.
	 *
	 * @param Document $document
	 */
	...
	public function on_status_change( $new_status, $old_status, $post ) {
		...
		// If it's from madxartwork editor, the usage should be saved via `before_document_save`/`after_document_save`.
	 ...
	 * Add's tracking data.
	 *
	 * Called on madxartwork/tracker/send_tracking_data_params.
	 *
	 * @param array $params
	 *
	 * @return array
	 ...
	 * Recalculate usage.
	 *
	 * Recalculate usage for all madxartwork posts.
	 *
	 * @param int $limit
	 * @param int $offset
	 *
	...
	private function save_document_usage( Document $document ) {
		...
		// Get data manually to avoid conflict with `\madxartwork\Core\Base\Document::get_elements_data... convert_to_madxartwork`.
		$data = $document->get_json_meta( '_madxartwork_data' );
	 ...
	 *
	 * Initializing madxartwork usage module.
	 *
	 * @access public
	 */
	public function __construct() {
		...
		add_action( 'madxartwork/document/before_save', [ $this, 'before_document_save' ] );
		add_action( 'madxartwork/document/after_save', [ $this, 'after_document_save' ] );

		add_filter( 'madxartwork/tracker/send_tracking_data_params', [ $this, 'add_tracking_data' ] );
