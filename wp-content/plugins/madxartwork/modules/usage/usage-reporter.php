...
/**
 * madxartwork usage report.
 *
 * madxartwork system report handler class responsible for generating a report for
 * the user.
 */
class Usage_Reporter extends Base_Reporter {

	const RECALC_ACTION = 'madxartwork_usage_recalc';
	...
	public function get_title() {
		...
		if ( 'html' === $this->_properties['format'] && empty( $_GET[ self::RECALC_ACTION ] ) ) { // phpcs:ignore -- nonce validation is not require here.
			...
			$title .= '<a id="madxartwork-usage-recalc" href="' . $url . '#madxartwork-usage-recalc" class="box-title-tool">Recalc</a>';
		}
