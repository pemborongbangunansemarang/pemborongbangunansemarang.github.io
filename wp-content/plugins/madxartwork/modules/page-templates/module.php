<?php
namespace madxartwork\Modules\PageTemplates;
...
use madxartwork\Core\DocumentTypes\PageBase as PageBase;
use madxartwork\Modules\Library\Documents\Page as LibraryPageDocument;
...
/**
 * madxartwork page templates module.
 *
 * madxartwork page templates module handler class is responsible for registering
 * and managing madxartwork page templates modules.
 *
 * @since 2.0.0
 */
class Module extends BaseModule {
	...
	/**
	 * madxartwork Canvas template name.
	 */
	const TEMPLATE_CANVAS = 'madxartwork_canvas';

	/**
	 * madxartwork Header & Footer template name.
	 */
	const TEMPLATE_HEADER_FOOTER = 'madxartwork_header_footer';
	 ...
	 * Template include.
	 *
	 * Update the path for the madxartwork Canvas template.
	 *
	 ...
	 *
	 * Adds madxartwork templates to all the post types that support
	 * madxartwork.
	 *
	...
	public function add_wp_templates_support() {
		$post_types = get_post_types_by_support( 'madxartwork' );
	 ...
	 * Add page templates.
	 *
	 * Add the madxartwork page templates to the theme templates.
	 *
	...
	public function add_page_templates( $page_templates, $wp_theme, $post ) {
		...
		$page_templates = [
			self::TEMPLATE_CANVAS => _x( 'madxartwork Canvas', 'Page Template', 'madxartwork' ),
			self::TEMPLATE_HEADER_FOOTER => _x( 'madxartwork Full Width', 'Page Template', 'madxartwork' ),
		] + $page_templates;
	...
	public function register_template_control( $document, $control_id = 'template' ) {
		...
		$document->add_control(
			$control_id,
			[
				'label' => __( 'Page Layout', 'madxartwork' ),
		...
		$document->add_control(
			...
			[
				...
				'raw' => __( 'Default Page Template from your theme', 'madxartwork' ),
				'separator' => 'none',
		...
		$document->add_control(
			...
			[
				'type' => Controls_Manager::RAW_HTML,
				'raw' => __( 'No header, no footer, just madxartwork', 'madxartwork' ),
				'separator' => 'none',
				'content_classes' => 'madxartwork-descriptor',
				'condition' => [
		...
		$document->add_control(
			...
			[
				'type' => Controls_Manager::RAW_HTML,
				'raw' => __( 'This template includes the header, full-width content and footer', 'madxartwork' ),
				'separator' => 'none',
				'content_classes' => 'madxartwork-descriptor',
				'condition' => [
	 ...
	 *
	 * madxartwork don't allow WordPress to update the parent page template
	 ...
	 *
	 * Initializing madxartwork page templates module.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function __construct() {
		...
		add_action( 'madxartwork/documents/register_controls', [ $this, 'action_register_template_control' ] );
