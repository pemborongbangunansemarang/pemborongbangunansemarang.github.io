 ...
 * madxartwork utils.
 *
 * madxartwork utils handler class is responsible for different utility methods
 * used by madxartwork.
 *
 * @since 1.0.0
 */
class Utils {
	 ...
	 * Get edit link.
	 *
	 * Retrieve madxartwork edit link.
	 *
	 * @since 1.0.0
	...
	public static function get_edit_link( $post_id = 0 ) {
		 ...
		 * Get edit link.
		 *
		 * Filters the madxartwork edit link.
		 *
		 * @since 1.0.0
		 * @deprecated 2.0.0 Use `madxartwork/document/urls/edit` filter instead.
		 *
		 * @param string $edit_link New URL query string (unescaped).
		 * @param int    $post_id   Post ID.
		 */
		$edit_link = apply_filters_deprecated( 'madxartwork/utils/get_edit_link', [ $edit_link, $post_id ], '2.0.0', 'madxartwork/document/urls/edit' );
	 ...
	 * Get pro link.
	 *
	 * Retrieve the link to madxartwork Pro.
	 *
	 * @since 1.7.0
	 * @access public
	 * @static
	 *
	 * @param string $link URL to madxartwork pro.
	 *
	 * @return string madxartwork pro link.
	 */
	...
	public static function get_preview_url( $post_id ) {
		 ...
		 * Preview URL.
		 *
		 * Filters the madxartwork preview URL.
		 *
		 * @since 1.6.4
		 * @deprecated 2.0.0 Use `madxartwork/document/urls/preview` filter instead.
		 *
		 * @param string $preview_url URL with chosen scheme.
		 * @param int    $post_id     Post ID.
		 */
		$url = apply_filters_deprecated( 'madxartwork/utils/preview_url', [ $url, $post_id ], '2.0.0', 'madxartwork/document/urls/preview' );
	...
	public static function get_wp_preview_url( $post_id ) {
		 ...
		 *
		 * @since 1.9.0
		 * @deprecated 2.0.0 Use `madxartwork/document/urls/wp_preview` filter instead.
		 *
		 * @param string $wp_preview_url WordPress preview URL.
		 * @param int    $post_id        Post ID.
		 */
		$wp_preview_url = apply_filters_deprecated( 'madxartwork/utils/wp_preview_url', [ $wp_preview_url, $post_id ], '2.0.0', 'madxartwork/document/urls/wp_preview' );
	 ...
	 * Replace URLs.
	 *
	 * Replace old URLs to new URLs. This method also updates all the madxartwork data.
	 *
	 * @since 2.1.0
	 * @static
	 * @access public
	...
	public static function replace_urls( $from, $to ) {
		...
		if ( $from === $to ) {
			throw new \Exception( __( 'The `from` and `to` URL\'s must be different', 'madxartwork' ) );
		}

		$is_valid_urls = ( filter_var( $from, FILTER_VALIDATE_URL ) && filter_var( $to, FILTER_VALIDATE_URL ) );
		if ( ! $is_valid_urls ) {
			throw new \Exception( __( 'The `from` and `to` URL\'s must be valid URL\'s', 'madxartwork' ) );
		}
		...
		$rows_affected = $wpdb->query(
			...
			"WHERE `meta_key` = '_madxartwork_data' AND `meta_value` LIKE '[%' ;" ); // meta_value LIKE '[%' are json formatted
		// @codingStandardsIgnoreEnd

		if ( false === $rows_affected ) {
			throw new \Exception( __( 'An error occurred', 'madxartwork' ) );
		}
		...
		return sprintf(
			...
			_n( '%d row affected.', '%d rows affected.', $rows_affected, 'madxartwork' ),
			$rows_affected
		);
	}
	...
	/**
	 * Is post supports madxartwork.
	 *
	 * Whether the post supports editing with madxartwork.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	 ...
	 * @param int $post_id Optional. Post ID. Default is `0`.
	 *
	 * @return string True if post supports editing with madxartwork, false otherwise.
	 */
	public static function is_post_support( $post_id = 0 ) {
		 ...
		 *
		 * Filters whether the post type supports editing with madxartwork.
		 *
		 * @since 1.0.0
		 * @deprecated 2.2.0 Use `madxartwork/utils/is_post_support` Instead
		 *
		 * @param bool $is_supported Whether the post type supports editing with madxartwork.
		 * @param int $post_id Post ID.
		 * @param string $post_type Post type.
		 */
		$is_supported = apply_filters( 'madxartwork/utils/is_post_type_support', $is_supported, $post_id, $post_type );

		/**
		 * Is post support.
		 *
		 * Filters whether the post supports editing with madxartwork.
		 *
		 * @since 2.2.0
		 *
		 * @param bool $is_supported Whether the post type supports editing with madxartwork.
		 ...
		 * @param string $post_type Post type.
		 */
		$is_supported = apply_filters( 'madxartwork/utils/is_post_support', $is_supported, $post_id, $post_type );
	...
	/**
	 * Is post type supports madxartwork.
	 *
	 * Whether the post type supports editing with madxartwork.
	 *
	 * @since 2.2.0
	 * @access public
	 * @static
	 ...
	 * @param string $post_type Post Type.
	 *
	 * @return string True if post type supports editing with madxartwork, false otherwise.
	 */
	public static function is_post_type_support( $post_type ) {
		...
		if ( ! post_type_supports( $post_type, 'madxartwork' ) ) {
			return false;
		}
	 ...
	 * @static
	 *
	 * @return string The source of the default placeholder image used by madxartwork.
	 */
	public static function get_placeholder_image_src() {
		$placeholder_image = madxartwork_ASSETS_URL . 'images/placeholder.png';

		/**
		 * Get placeholder image source.
		 *
		 * Filters the source of the default placeholder image used by madxartwork.
		 *
		 * @since 1.0.0
		 *
		 * @param string $placeholder_image The source of the default placeholder image.
		 */
		$placeholder_image = apply_filters( 'madxartwork/utils/get_placeholder_image_src', $placeholder_image );
	 ...
	 *
	 * Retrieve a custom URL for creating a new post/page using madxartwork.
	 *
	 * @since 1.9.0
	 * @access public
	 * @static
	 ...
	 *
	 * @return string A URL for creating new post using madxartwork.
	 */
	public static function get_create_new_post_url( $post_type = 'page' ) {
		...
		$new_post_url = add_query_arg( '_wpnonce', wp_create_nonce( 'madxartwork_action_new_post' ), $new_post_url );
	...
	public static function get_meta_viewport( $context = '' ) {
		 ...
		 */
		return apply_filters( 'madxartwork/template/viewport_tag', $meta_tag, $context );
	}

	/**
	 * Add madxartwork Config js vars to the relevant script handle,
	...
	public static function print_js_config( $handle, $js_var, $config ) {
		...
		if ( get_option( 'madxartwork_editor_break_lines' ) ) {
	...
	public static function handle_deprecation( $item, $version, $replacement = null ) {
		preg_match( '/^[0-9]+\.[0-9]+/', madxartwork_VERSION, $current_version );
