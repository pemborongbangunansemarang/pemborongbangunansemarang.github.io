<?php
namespace madxartwork\TemplateLibrary;
...
use madxartwork\Core\Settings\Page\Model;
...
/**
 * madxartwork template library local source.
 *
 * madxartwork template library local source handler class is responsible for
 * handling local madxartwork templates saved by the user locally on his site.
 *
 * @since 1.0.0
 */
class Source_Local extends Source_Base {
	...
	/**
	 * madxartwork template-library post-type slug.
	 */
	const CPT = 'madxartwork_library';

	/**
	 * madxartwork template-library taxonomy slug.
	 */
	const TAXONOMY_TYPE_SLUG = 'madxartwork_library_type';

	/**
	 * madxartwork template-library category slug.
	 */
	const TAXONOMY_CATEGORY_SLUG = 'madxartwork_library_category';

	/**
	 * madxartwork template-library meta key.
	 * @deprecated 2.3.0 Use \madxartwork\Core\Base\Document::TYPE_META_KEY instead
	 */
	const TYPE_META_KEY = '_madxartwork_template_type';

	/**
	 * madxartwork template-library temporary files folder.
	 */
	const TEMP_FILES_DIR = 'madxartwork/tmp';

	/**
	 * madxartwork template-library bulk export action name.
	 */
	const BULK_EXPORT_ACTION = 'madxartwork_export_multiple_templates';

	const ADMIN_MENU_SLUG = 'edit.php?post_type=madxartwork_library';

	const ADMIN_SCREEN_ID = 'edit-madxartwork_library';
	 ...
	 * other data.
	 *
	 * The local template class registers a new `madxartwork_library` post type
	 * and an `madxartwork_library_type` taxonomy. They are used to store data for
	...
	public function register_data() {
		$labels = [
			'name' => _x( 'My Templates', 'Template Library', 'madxartwork' ),
			'singular_name' => _x( 'Template', 'Template Library', 'madxartwork' ),
			'add_new' => _x( 'Add New', 'Template Library', 'madxartwork' ),
			'add_new_item' => _x( 'Add New Template', 'Template Library', 'madxartwork' ),
			'edit_item' => _x( 'Edit Template', 'Template Library', 'madxartwork' ),
			'new_item' => _x( 'New Template', 'Template Library', 'madxartwork' ),
			'all_items' => _x( 'All Templates', 'Template Library', 'madxartwork' ),
			'view_item' => _x( 'View Template', 'Template Library', 'madxartwork' ),
			'search_items' => _x( 'Search Template', 'Template Library', 'madxartwork' ),
			'not_found' => _x( 'No Templates found', 'Template Library', 'madxartwork' ),
			'not_found_in_trash' => _x( 'No Templates found in Trash', 'Template Library', 'madxartwork' ),
			'parent_item_colon' => '',
			'menu_name' => _x( 'Templates', 'Template Library', 'madxartwork' ),
		];
		...
		$args = [
			...
			'hierarchical' => false,
			'supports' => [ 'title', 'thumbnail', 'author', 'madxartwork' ],
		];

		/**
		 * Register template library post type args.
		 *
		 * Filters the post type arguments when registering madxartwork template library post type.
		 *
		 * @since 1.0.0
		 *
		 * @param array $args Arguments for registering a post type.
		 */
		$args = apply_filters( 'madxartwork/template_library/sources/local/register_post_type_args', $args );
		...
		$args = [
			...
			'rewrite' => false,
			'public' => false,
			'label' => _x( 'Type', 'Template Library', 'madxartwork' ),
		];

		/**
		 * Register template library taxonomy args.
		 *
		 * Filters the taxonomy arguments when registering madxartwork template library taxonomy.
		 *
		 * @since 1.0.0
		 *
		 * @param array $args Arguments for registering a taxonomy.
		 */
		$args = apply_filters( 'madxartwork/template_library/sources/local/register_taxonomy_args', $args );
		...
		$args = [
			...
			'public' => false,
			'labels' => [
				'name' => _x( 'Categories', 'Template Library', 'madxartwork' ),
				'singular_name' => _x( 'Category', 'Template Library', 'madxartwork' ),
				'all_items' => _x( 'All Categories', 'Template Library', 'madxartwork' ),
			],
		];
		 ...
		 *
		 * Filters the category arguments when registering madxartwork template library category.
		 *
		 * @since 2.4.0
		 *
		 * @param array $args Arguments for registering a category.
		 */
		$args = apply_filters( 'madxartwork/template_library/sources/local/register_category_args', $args );
	...
	public function admin_menu() {
		add_submenu_page( self::ADMIN_MENU_SLUG, '', __( 'Saved Templates', 'madxartwork' ), Editor::EDITING_CAPABILITY, self::get_admin_url( true ) );
	}
	...
	public function save_item( $template_data ) {
		if ( ! current_user_can( $this->post_type_object->cap->edit_posts ) ) {
			return new \WP_Error( 'save_error', __( 'Access denied.', 'madxartwork' ) );
		}

		$defaults = [
			'title' => __( '(no title)', 'madxartwork' ),
			'page_settings' => [],
		 ...
		 *
		 * Fires after madxartwork template library was saved.
		 *
		 * @since 1.0.1
		 *
		 ...
		 */
		do_action( 'madxartwork/template-library/after_save_template', $template_id, $template_data );

		/**
		 * After template library update.
		 *
		 * Fires after madxartwork template library was updated.
		 *
		 * @since 1.0.1
		 *
		 * @param int   $template_id   The ID of the template.
		 * @param array $template_data The template data.
		 */
		do_action( 'madxartwork/template-library/after_update_template', $template_id, $template_data );
	...
	public function update_item( $new_data ) {
		...
		if ( ! $document ) {
			return new \WP_Error( 'save_error', __( 'Template not exist.', 'madxartwork' ) );
		}
		 ...
		 */
		do_action( 'madxartwork/template-library/after_update_template', $new_data['id'], $new_data );
	...
	public function get_item( $template_id ) {
		 ...
		 */
		$data = apply_filters( 'madxartwork/template-library/get_template', $data );
	...
	public function delete_template( $template_id ) {
		if ( ! current_user_can( $this->post_type_object->cap->delete_post, $template_id ) ) {
			return new \WP_Error( 'template_error', __( 'Access denied.', 'madxartwork' ) );
		}
	...
	public function export_multiple_templates( array $template_ids ) {
		...
		$zip_archive_filename = 'madxartwork-templates-' . date( 'Y-m-d' ) . '.zip';
	...
	public function post_row_actions( $actions, \WP_Post $post ) {
		if ( self::is_base_templates_screen() ) {
			if ( $this->is_template_supports_export( $post->ID ) ) {
				$actions['export-template'] = sprintf( '<a href="%1$s">%2$s</a>', $this->get_export_link( $post->ID ), __( 'Export Template', 'madxartwork' ) );
			}
		}
	...
	public function admin_import_template_form() {
		...
		/** @var \madxartwork\Core\Common\Modules\Ajax\Module $ajax */
		$ajax = Plugin::$instance->common->get_component( 'ajax' );
		?>
		<div id="madxartwork-hidden-area">
			<a id="madxartwork-import-template-trigger" class="page-title-action"><?php echo __( 'Import Templates', 'madxartwork' ); ?></a>
			<div id="madxartwork-import-template-area">
				<div id="madxartwork-import-template-title"><?php echo __( 'Choose an madxartwork template JSON file or a .zip archive of madxartwork templates, and add them to the list of templates available in your library.', 'madxartwork' ); ?></div>
				<form id="madxartwork-import-template-form" method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" enctype="multipart/form-data">
					<input type="hidden" name="action" value="madxartwork_library_direct_actions">
					...
					<fieldset id="madxartwork-import-template-form-inputs">
						...
						<input type="submit" class="button" value="<?php echo esc_attr__( 'Import Now', 'madxartwork' ); ?>">
					</fieldset>
				</form>
			</div>
		</div>
	...
	public function is_template_supports_export( $template_id ) {
		 ...
		 */
		$export_support = apply_filters( 'madxartwork/template_library/is_template_supports_export', $export_support, $template_id );

		return $export_support;
	}

	/**
	 * Remove madxartwork post state.
	 *
	 * Remove the 'madxartwork' post state from the display states of the post.
	 *
	 * Used to remove the 'madxartwork' post state from the template library items.
	 *
	 ...
	 */
	public function remove_madxartwork_post_state_from_library( $post_states, $post ) {
		if ( self::CPT === $post->post_type && isset( $post_states['madxartwork'] ) ) {
			unset( $post_states['madxartwork'] );
		}
		return $post_states;
	}
	...
	private function get_export_link( $template_id ) {
		...
		return add_query_arg(
			[
				'action' => 'madxartwork_library_direct_actions',
				'library_action' => 'export_template',
				'source' => $this->get_id(),
				'_nonce' => wp_create_nonce( 'madxartwork_ajax' ),
	 ...
	 * library.
	 *
	 * Fired by `bulk_actions-edit-madxartwork_library` filter.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	...
	public function admin_add_bulk_export_action( $actions ) {
		$actions[ self::BULK_EXPORT_ACTION ] = __( 'Export', 'madxartwork' );
	 ...
	 *
	 * Fired by `handle_bulk_actions-edit-madxartwork_library` filter.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	 ...
	 *
	 * Fired by `views_edit-madxartwork_library` filter.
	 *
	 * @since 2.0.0
	 * @access public
	 *
	...
	public function admin_print_tabs( $views ) {
		...
		?>
		<div id="madxartwork-template-library-tabs-wrapper" class="nav-tab-wrapper">
			<a class="nav-tab<?php echo $active_class; ?>" href="<?php echo $baseurl; ?>">
				<?php
				$all_title = $this->get_library_title();
				if ( ! $all_title ) {
					$all_title = __( 'All', 'madxartwork' );
				}
				echo $all_title; ?>
			</a>
			<?php
	...
	public function maybe_render_blank_state( $which ) {
		...
		$current_type = get_query_var( 'madxartwork_library_type' );
		...
		if ( empty( $current_type ) ) {
			...
			$inline_style .= '#madxartwork-template-library-tabs-wrapper {display: none;}';
		}

		$current_type_label = $this->get_template_label_by_type( $current_type );
		?>
		<style type="text/css"><?php echo $inline_style; ?></style>
		<div class="madxartwork-template_library-blank_state">
			<div class="madxartwork-blank_state">
				<i class="eicon-folder"></i>
				<h2>
					<?php
					/* translators: %s: Template type label. */
					printf( __( 'Create Your First %s', 'madxartwork' ), $current_type_label );
					?>
				</h2>
				<p><?php echo __( 'Add templates and reuse them across your website. Easily export and import them to any other project, for an optimized workflow.', 'madxartwork' ); ?></p>
				<a id="madxartwork-template-library-add-new" class="madxartwork-button madxartwork-button-success" href="<?php esc_url( Utils::get_pro_link( 'https://madxartwork.com/pro/?utm_source=wp-custom-fonts&utm_campaign=gopro&utm_medium=wp-dash' ) ); ?>">
					<?php
					/* translators: %s: Template type label. */
					printf( __( 'Add New %s', 'madxartwork' ), $current_type_label );
					?>
				</a>
			</div>
		</div>
	...
	public function add_filter_by_category( $post_type ) {
		...
		echo '<label class="screen-reader-text" for="cat">' . _x( 'Filter by category', 'Template Library', 'madxartwork' ) . '</label>';
	...
	private function prepare_template_export( $template_id ) {
		...
		if ( get_post_meta( $template_id, '_madxartwork_page_settings', true ) ) {
			$page = SettingsManager::get_settings_managers( 'page' )->get_model( $template_id );
		...
		return [
			'name' => 'madxartwork-' . $template_id . '-' . date( 'Y-m-d' ) . '.json',
	...
	private function get_template_label_by_type( $template_type ) {
		 ...
		 * @param string $template_type  Template type.
		 */
		$template_label = apply_filters( 'madxartwork/template-library/get_template_label_by_type', $template_label, $template_type );
	...
	private function add_actions() {
		if ( is_admin() ) {
			...
			add_filter( 'display_post_states', [ $this, 'remove_madxartwork_post_state_from_library' ], 11, 2 );
			...
			add_filter( 'bulk_actions-edit-madxartwork_library', [ $this, 'admin_add_bulk_export_action' ] );
			add_filter( 'handle_bulk_actions-edit-madxartwork_library', [ $this, 'admin_export_multiple_templates' ], 10, 3 );
	...
	public function admin_columns_content( $column_name, $post_id ) {
		if ( 'madxartwork_library_type' === $column_name ) {
	...
	public function admin_columns_headers( $posts_columns ) {
		...
		unset( $posts_columns['taxonomy-madxartwork_library_type'] );

		$offset = 2;

		$posts_columns = array_slice( $posts_columns, 0, $offset, true ) + [
			'madxartwork_library_type' => __( 'Type', 'madxartwork' ),
	...
	private function get_library_title() {
		...
		if ( $this->is_current_screen() ) {
			...
			if ( $current_tab_group ) {
				$titles = [
					'library' => __( 'Saved Templates', 'madxartwork' ),
					'theme' => __( 'Theme Builder', 'madxartwork' ),
					'popup' => __( 'Popups', 'madxartwork' ),
				];
