...
/**
 * madxartwork alert widget.
 *
 * madxartwork widget that displays a collapsible display of content in an toggle
...
class Widget_Alert extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'This is an Alert', 'madxartwork' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'I am a description. Click the edit button to change this text.', 'madxartwork' ),
				'separator' => 'none',
				'show_label' => false,
			]
		);
		...
		$this->add_control(
			'show_dismiss',
			[
				'label' => __( 'Dismiss Button', 'madxartwork' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'show',
				'options' => [
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-alert' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-alert' => 'border-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			'border_left-width',
			[
				'label' => __( 'Left Border Width', 'madxartwork' ),
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-alert' => 'border-left-width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-alert-title' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'alert_title',
				'selector' => '{{WRAPPER}} .madxartwork-alert-title',
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-alert-description' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-alert-description',
	...
	protected function render() {
		...
		if ( ! empty( $settings['alert_type'] ) ) {
			$this->add_render_attribute( 'wrapper', 'class', 'madxartwork-alert madxartwork-alert-' . $settings['alert_type'] );
		}

		$this->add_render_attribute( 'wrapper', 'role', 'alert' );

		$this->add_render_attribute( 'alert_title', 'class', 'madxartwork-alert-title' );
		...
		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			...
			if ( ! Utils::is_empty( $settings['alert_description'] ) ) :
				$this->add_render_attribute( 'alert_description', 'class', 'madxartwork-alert-description' );
			...
			<?php if ( 'show' === $settings['show_dismiss'] ) : ?>
				<button type="button" class="madxartwork-alert-dismiss">
					<span aria-hidden="true">&times;</span>
					<span class="madxartwork-screen-only"><?php echo __( 'Dismiss alert', 'madxartwork' ); ?></span>
				</button>
			<?php endif; ?>
		</div>
		<?php
	...
	protected function _content_template() {
		...
		<# if ( settings.alert_title ) {
			view.addRenderAttribute( {
				alert_title: { class: 'madxartwork-alert-title' },
				alert_description: { class: 'madxartwork-alert-description' }
			} );

			view.addInlineEditingAttributes( 'alert_title', 'none' );
			view.addInlineEditingAttributes( 'alert_description' );
			#>
			<div class="madxartwork-alert madxartwork-alert-{{ settings.alert_type }}" role="alert">
