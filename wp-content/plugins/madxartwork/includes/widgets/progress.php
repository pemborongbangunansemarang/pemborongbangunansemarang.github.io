...
/**
 * madxartwork progress widget.
 *
 * madxartwork widget that displays an escalating progress bar.
 *
 * @since 1.0.0
 */
class Widget_Progress extends Widget_Base {
	...
	public function get_title() {
		return __( 'Progress Bar', 'madxartwork' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_progress',
			[
				'label' => __( 'Progress Bar', 'madxartwork' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => __( 'My Skill', 'madxartwork' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			'percent',
			[
				'label' => __( 'Percentage', 'madxartwork' ),
		...
		$this->add_control( 'display_percentage', [
			'label' => __( 'Display Percentage', 'madxartwork' ),
			'type' => Controls_Manager::SELECT,
			'default' => 'show',
			'options' => [
				'show' => __( 'Show', 'madxartwork' ),
				'hide' => __( 'Hide', 'madxartwork' ),
			],
		] );
		...
		$this->add_control(
			'inner_text',
			[
				'label' => __( 'Inner Text', 'madxartwork' ),
				'type' => Controls_Manager::TEXT,
				'dynamic' => [
					'active' => true,
				],
				'placeholder' => __( 'e.g. Web Designer', 'madxartwork' ),
				'default' => __( 'Web Designer', 'madxartwork' ),
				'label_block' => true,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-progress-wrapper .madxartwork-progress-bar' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-progress-wrapper' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-progress-bar' => 'height: {{SIZE}}{{UNIT}}; line-height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .madxartwork-progress-wrapper' => 'border-radius: {{SIZE}}{{UNIT}}; overflow: hidden;',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-progress-bar' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-progress-bar',
				'exclude' => [
					'line_height',
				],
			]
		...
		$this->start_controls_section(
			'section_title',
			[
				'label' => __( 'Title Style', 'madxartwork' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-title' => 'color: {{VALUE}};',
				],
				'scheme' => [
					'type' => Scheme_Color::get_type(),
					'value' => Scheme_Color::COLOR_1,
		...
		$this->add_group_control(
			...
			[
				'name' => 'typography',
				'selector' => '{{WRAPPER}} .madxartwork-title',
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', [
			'class' => 'madxartwork-progress-wrapper',
			'role' => 'progressbar',
			'aria-valuemin' => '0',
			'aria-valuemax' => '100',
		...
		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			<div <?php echo $this->get_render_attribute_string( 'progress-bar' ); ?>>
				...
				<?php if ( 'hide' !== $settings['display_percentage'] ) { ?>
					<span class="madxartwork-progress-percentage"><?php echo $progress_percentage; ?>%</span>
				<?php } ?>
			</div>
		</div>
		<?php
	...
	protected function _content_template() {
		...
		view.addRenderAttribute( 'progressWrapper', {
			'class': [ 'madxartwork-progress-wrapper', 'progress-' + settings.progress_type ],
			'role': 'progressbar',
			'aria-valuemin': '0',
			'aria-valuemax': '100',
			'aria-valuenow': progress_percentage,
		...
		#>
		<# if ( settings.title ) { #>
			<span class="madxartwork-title">{{{ settings.title }}}</span><#
		} #>
		<div {{{ view.getRenderAttributeString( 'progressWrapper' ) }}}>
			<div class="madxartwork-progress-bar" data-max="{{ progress_percentage }}">
				<span {{{ view.getRenderAttributeString( 'inner_text' ) }}}>{{{ settings.inner_text }}}</span>
				<# if ( 'hide' !== settings.display_percentage ) { #>
					<span class="madxartwork-progress-percentage">{{{ progress_percentage }}}%</span>
				<# } #>
			</div>
		</div>
		<?php
