...
/**
 * madxartwork image widget.
 *
 * madxartwork widget that displays an image into the page.
 *
 * @since 1.0.0
 */
class Widget_Image extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'attachment' => __( 'Attachment Caption', 'madxartwork' ),
					'custom' => __( 'Custom Caption', 'madxartwork' ),
				],
				'default' => 'none',
			]
		);
		...
		$this->add_control(
			'caption',
			[
				'label' => __( 'Custom Caption', 'madxartwork' ),
				'type' => Controls_Manager::TEXT,
				'default' => '',
				'placeholder' => __( 'Enter your image caption', 'madxartwork' ),
				'condition' => [
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-image img' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			'space',
			[
				'label' => __( 'Max Width', 'madxartwork' ) . ' (%)',
				'type' => Controls_Manager::SLIDER,
				'default' => [
					'unit' => '%',
				],
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-image img' => 'max-width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-image img' => 'opacity: {{SIZE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'css_filters',
				'selector' => '{{WRAPPER}} .madxartwork-image img',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-image:hover img' => 'opacity: {{SIZE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-image:hover img',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-image img' => 'transition-duration: {{SIZE}}s',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .madxartwork-image img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'madxartwork-image' );

		if ( ! empty( $settings['shape'] ) ) {
			$this->add_render_attribute( 'wrapper', 'class', 'madxartwork-image-shape-' . $settings['shape'] );
		}
	...
	protected function _content_template() {
		...
		<# if ( settings.image.url ) {
			...
			#><div class="madxartwork-image{{ settings.shape ? ' madxartwork-image-shape-' + settings.shape : '' }}"><#
			var imgClass = '';

			if ( '' !== settings.hover_animation ) {
				imgClass = 'madxartwork-animation-' + settings.hover_animation;
			}
			...
			if ( link_url ) {
					#><a class="madxartwork-clickable" data-madxartwork-open-lightbox="{{ settings.open_lightbox }}" href="{{ link_url }}"><#
			}
