...
/**
 * madxartwork star rating widget.
 *
 * madxartwork widget that displays star rating.
 *
 * @since 2.3.0
 */
class Widget_Star_Rating extends Widget_Base {
	...
	public function get_title() {
		return __( 'Star Rating', 'madxartwork' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'rating_scale',
			[
				'label' => __( 'Rating Scale', 'madxartwork' ),
		...
		$this->add_control(
			'unmarked_star_style',
			[
				'label' => __( 'Unmarked Style', 'madxartwork' ),
				'type' => Controls_Manager::CHOOSE,
				'label_block' => false,
				'options' => [
					'solid' => [
						'title' => __( 'Solid', 'madxartwork' ),
						'icon' => 'eicon-star',
					],
					'outline' => [
						'title' => __( 'Outline', 'madxartwork' ),
						'icon' => 'eicon-star-o',
					],
				],
				'default' => 'solid',
		...
		$this->add_responsive_control(
			...
			[
				...
				'prefix_class' => 'madxartwork-star-rating%s--align-',
				'selectors' => [
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-star-rating__title' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-star-rating__title',
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'body:not(.rtl) {{WRAPPER}}:not(.madxartwork-star-rating--align-justify) .madxartwork-star-rating__title' => 'margin-right: {{SIZE}}{{UNIT}}',
					'body.rtl {{WRAPPER}}:not(.madxartwork-star-rating--align-justify) .madxartwork-star-rating__title' => 'margin-left: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => __( 'Unmarked Color', 'madxartwork' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
	...
	protected function render() {
		...
		$schema_rating = '<span itemprop="ratingValue" class="madxartwork-screen-only">' . $textual_rating . '</span>';
		$stars_element = '<div ' . $this->get_render_attribute_string( 'icon_wrapper' ) . '>' . $this->render_stars( $icon ) . ' ' . $schema_rating . '</div>';
		?>

		<div class="madxartwork-star-rating__wrapper">
			<?php if ( ! Utils::is_empty( $settings['title'] ) ) : ?>
				<div class="madxartwork-star-rating__title"><?php echo $settings['title']; ?></div>
			<?php endif; ?>
	...
	protected function _content_template() {
		...
		<#
			...
			renderStars = function( icon ) {
				...
				for ( var stars = 1; stars <= ratingData[1]; stars++ ) {
					if ( stars <= flooredRating  ) {
						starsHtml += '<i class="madxartwork-star-full">' + icon + '</i>';
					} else if ( flooredRating + 1 === stars && rating !== flooredRating ) {
						starsHtml += '<i class="madxartwork-star-' + ( rating - flooredRating ).toFixed( 1 ) * 10 + '">' + icon + '</i>';
					} else {
						starsHtml += '<i class="madxartwork-star-empty">' + icon + '</i>';
					}
				}
			...
			view.addRenderAttribute( 'iconWrapper', 'class', 'madxartwork-star-rating' );
		...
		<div class="madxartwork-star-rating__wrapper">
			<# if ( ! _.isEmpty( settings.title ) ) { #>
				<div class="madxartwork-star-rating__title">{{ settings.title }}</div>
			<# } #>
			<div {{{ view.getRenderAttributeString( 'iconWrapper' ) }}} >
				{{{ stars }}}
				<span itemprop="ratingValue" class="madxartwork-screen-only">{{ textualRating }}</span>
			</div>
		</div>
