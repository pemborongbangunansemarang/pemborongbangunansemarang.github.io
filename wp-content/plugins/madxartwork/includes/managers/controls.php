...
/**
 * madxartwork controls manager.
 *
 * madxartwork controls manager handler class is responsible for registering and
...
class Controls_Manager {
	...
	private static function init_tabs() {
		self::$tabs = [
			self::TAB_CONTENT => __( 'Content', 'madxartwork' ),
			self::TAB_STYLE => __( 'Style', 'madxartwork' ),
			self::TAB_ADVANCED => __( 'Advanced', 'madxartwork' ),
			self::TAB_RESPONSIVE => __( 'Responsive', 'madxartwork' ),
			self::TAB_LAYOUT => __( 'Layout', 'madxartwork' ),
			self::TAB_SETTINGS => __( 'Settings', 'madxartwork' ),
		];
	}
	 ...
	 * `madxartwork/controls/controls_registered` action.
	 *
	 * @since 1.0.0
	 * @access private
	 */
	private function register_controls() {
		 ...
		 *
		 * Fires after madxartwork controls are registered.
		 *
		 * @since 1.0.0
		 *
		 * @param Controls_Manager $this The controls manager.
		 */
		do_action( 'madxartwork/controls/controls_registered', $this );
	}
	 ...
	 * version of madxartwork uses this method to display an upgrade message to
	 * madxartwork Pro.
	 *
	 * @since 1.0.0
	 * @access public
	...
	public function add_custom_css_controls( Controls_Stack $controls_stack ) {
		$controls_stack->start_controls_section(
			...
			[
				'label' => __( 'Custom CSS', 'madxartwork' ),
		...
		$controls_stack->add_control(
			...
			[
				'type' => self::RAW_HTML,
				'raw' => '<div class="madxartwork-nerd-box">' .
						'<i class="madxartwork-nerd-box-icon eicon-hypster" aria-hidden="true"></i>
						<div class="madxartwork-nerd-box-title">' .
							__( 'Meet Our Custom CSS', 'madxartwork' ) .
						'</div>
						<div class="madxartwork-nerd-box-message">' .
							__( 'Custom CSS lets you add CSS code to any widget, and see it render live right in the editor.', 'madxartwork' ) .
						'</div>
						<div class="madxartwork-nerd-box-message">' .
							__( 'This feature is only available on madxartwork Pro.', 'madxartwork' ) .
						'</div>
						<a class="madxartwork-nerd-box-link madxartwork-button madxartwork-button-default madxartwork-go-pro" href="' . Utils::get_pro_link( 'https://madxartwork.com/pro/?utm_source=panel-custom-css&utm_campaign=gopro&utm_medium=wp-dash' ) . '" target="_blank">' .
							__( 'Go Pro', 'madxartwork' ) .
						'</a>
						</div>',
			]
		);
