...
class Add_To_Cart extends Widget_Button {
	...
	public function get_title() {
		return __( 'Custom Add To Cart', 'madxartwork-pro' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_product',
			[
				'label' => __( 'Product', 'madxartwork-pro' ),
			]
		);
		...
		$this->add_control(
			'show_quantity',
			[
				'label' => __( 'Show Quantity', 'madxartwork-pro' ),
				'type' => Controls_Manager::SWITCHER,
				'label_off' => __( 'Hide', 'madxartwork-pro' ),
				'label_on' => __( 'Show', 'madxartwork-pro' ),
				'description' => __( 'Please note that switching on this option will disable some of the design controls.', 'madxartwork-pro' ),
			]
		);
		...
		$this->add_control(
			'quantity',
			[
				'label' => __( 'Quantity', 'madxartwork-pro' ),
		...
		$this->update_control(
			'text',
			[
				'default' => __( 'Add to Cart', 'madxartwork-pro' ),
				'placeholder' => __( 'Add to Cart', 'madxartwork-pro' ),
			]
		);
	...
	private function render_ajax_button( $product ) {
		...
		if ( $product ) {
			...
			$settings['text'] = __( 'Please set a valid product', 'madxartwork-pro' );
	...
	private function render_form_button( $product ) {
		if ( ! $product && current_user_can( 'manage_options' ) ) {
			echo __( 'Please set a valid product', 'madxartwork-pro' );
		...
		$form = ob_get_clean();
		$form = str_replace( 'single_add_to_cart_button', 'single_add_to_cart_button madxartwork-button', $form );
		echo $form;
