...
namespace madxartworkPro\Modules\Woocommerce\WcTemplates\Cart;

defined( 'ABSPATH' ) || exit;

if ( ! function_exists( 'madxartwork_pro_render_mini_cart_item' ) ) {
	function madxartwork_pro_render_mini_cart_item( $cart_item_key, $cart_item ) {
...
<?php } else { ?>
	<div class="madxartwork-menu-cart__products woocommerce-mini-cart cart woocommerce-cart-form__contents">
		...
		foreach ( $cart_items as $cart_item_key => $cart_item ) {
			madxartwork_pro_render_mini_cart_item( $cart_item_key, $cart_item );
		}
	...
	<div class="madxartwork-menu-cart__footer-buttons">
		<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="madxartwork-button madxartwork-button--view-cart madxartwork-size-md">
			<span class="madxartwork-button-text"><?php echo __( 'View cart', 'woocommerce' ); // phpcs:ignore WordPress.WP.I18n ?></span>
		</a>
		<a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="madxartwork-button madxartwork-button--checkout madxartwork-size-md">
			<span class="madxartwork-button-text"><?php echo __( 'Checkout', 'woocommerce' ); // phpcs:ignore WordPress.WP.I18n ?></span>
		</a>
	</div>
	<?php
} // empty( $cart_items )
