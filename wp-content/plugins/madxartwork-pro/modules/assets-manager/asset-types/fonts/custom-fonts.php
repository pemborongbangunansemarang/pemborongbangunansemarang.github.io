<?php
namespace madxartworkPro\Modules\AssetsManager\AssetTypes\Fonts;

use madxartwork\Core\Files\CSS\Base;
use madxartworkPro\Modules\AssetsManager\Classes;
use madxartworkPro\Modules\AssetsManager\AssetTypes\Fonts_Manager;
...
class Custom_Fonts extends Classes\Font_Base {
	...
	const FONT_META_KEY = 'madxartwork_font_files';
	const FONT_FACE_META_KEY = 'madxartwork_font_face';

	public function get_name() {
		return __( 'Custom Fonts', 'madxartwork-pro' );
	}
	...
	public function add_meta_box() {
		add_meta_box(
			'madxartwork-font-' . $this->get_type() . 'metabox',
			__( 'Manage Your Font Files', 'madxartwork-pro' ),
			[ $this, 'render_metabox' ],
			Fonts_Manager::CPT,
			'normal',
			'default'
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			...
			[
				...
				'field_type' => 'html',
				'label' => false,
				'raw_html' => sprintf( '<div class="inline-preview">%s</div>', __( 'madxartwork Is Making the Web Beautiful!!!', 'madxartwork-pro' ) ),
			],
			[
				'id' => 'toolbar',
				'field_type' => 'toolbar',
		...
		foreach ( $this->get_file_types() as $type => $mine ) {
			$fields[] = [
				...
				'label' => sprintf( __( '%s File', 'madxartwork-pro' ), strtoupper( $type ) ),
				/* translators: %s: Font file format. */
				'box_title' => sprintf( __( 'Upload font .%s file', 'madxartwork-pro' ), $type ),
				/* translators: %s: Font file format. */
				'box_action' => sprintf( __( 'Select .%s file', 'madxartwork-pro' ), $type ),
				'preview_anchor' => 'none',
		...
		$repeater = [
			...
			'id' => 'font_face',
			'label' => false,
			'add_label' => __( 'Add Font Variation', 'madxartwork-pro' ),
			'toggle_title' => __( 'Edit', 'madxartwork-pro' ),
			'remove_title' => __( 'Delete', 'madxartwork-pro' ),
			'field_type' => 'repeater',
			'row_label' => [
				'default' => 'Settings',
	...
	public function upload_mimes( $mine_types ) {
		if ( current_user_can( Fonts_Manager::CAPABILITY ) && $this->is_madxartwork_font_upload() ) {
			foreach ( $this->get_file_types() as $type => $mine ) {
	...
	public function wp_handle_upload_prefilter( $file ) {
		if ( ! $this->is_madxartwork_font_upload() ) {
			return $file;
		}
		...
		/**
		 * @var \madxartwork\Core\Files\Assets\Svg\Svg_Handler $svg_handler;
		 */
		$svg_handler = Plugin::madxartwork()->assets_manager->get_asset( 'svg-handler' );

		if ( $svg_handler::svg_sanitizer_can_run() && ! $svg_handler->sanitize_svg( $file['tmp_name'] ) ) {
			$file['error'] = __( 'Invalid SVG Format, file not uploaded for security reasons', 'madxartwork-pro' );
		}

		return $file;
	}

	private function is_madxartwork_font_upload() {
		return isset( $_POST['uploadTypeCaller'] ) && 'madxartwork-admin-font-upload' === $_POST['uploadTypeCaller']; // phpcs:ignore
	}
	...
	public function get_font_face_from_data( $font_family, $data ) {
		...
		$font_face .= "\tfont-display: " . apply_filters( 'madxartwork_pro/custom_fonts/font_display', 'auto', $font_family, $data ) . ';' . PHP_EOL;
	...
	public function handle_panel_request( array $data ) {
		...
		if ( empty( $font_face ) ) {
			...
			$error_message = sprintf( __( 'Font %s was not found.', 'madxartwork-pro' ), $font_family );
	...
	private function get_font_style_options() {
		return [
			...
			'italic' => __( 'Italic', 'madxartwork-pro' ),
			'oblique' => __( 'Oblique', 'madxartwork-pro' ),
		];
	}
	...
	private function get_file_type_description( $file_type ) {
		$descriptions = [
			'eot' => __( 'Embedded OpenType, Used by IE6-IE9 Browsers', 'madxartwork-pro' ),
			'woff2' => __( 'The Web Open Font Format 2, Used by Super Modern Browsers', 'madxartwork-pro' ),
			'woff' => __( 'The Web Open Font Format, Used by Modern Browsers', 'madxartwork-pro' ),
			'ttf' => __( 'TrueType Fonts, Used for better supporting Safari, Android, iOS', 'madxartwork-pro' ),
			'svg' => __( 'SVG fonts allow SVG to be used as glyphs when displaying text, Used by Legacy iOS', 'madxartwork-pro' ),
		];
