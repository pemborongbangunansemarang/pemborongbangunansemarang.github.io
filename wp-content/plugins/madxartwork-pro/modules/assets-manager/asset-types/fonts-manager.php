...
class Fonts_Manager {
	...
	const CPT = 'madxartwork_font';

	const TAXONOMY = 'madxartwork_font_type';
	...
	/**
	 * Register madxartwork font custom post type and madxartwork font type custom taxonomy
	 */
	public function register_post_type_and_tax() {
		$labels = [
			'name' => _x( 'Custom Fonts', 'madxartwork Font', 'madxartwork-pro' ),
			'singular_name' => _x( 'Font', 'madxartwork Font', 'madxartwork-pro' ),
			'add_new' => _x( 'Add New', 'madxartwork Font', 'madxartwork-pro' ),
			'add_new_item' => _x( 'Add New Font', 'madxartwork Font', 'madxartwork-pro' ),
			'edit_item' => _x( 'Edit Font', 'madxartwork Font', 'madxartwork-pro' ),
			'new_item' => _x( 'New Font', 'madxartwork Font', 'madxartwork-pro' ),
			'all_items' => _x( 'All Fonts', 'madxartwork Font', 'madxartwork-pro' ),
			'view_item' => _x( 'View Font', 'madxartwork Font', 'madxartwork-pro' ),
			'search_items' => _x( 'Search Font', 'madxartwork Font', 'madxartwork-pro' ),
			'not_found' => _x( 'No Fonts found', 'madxartwork Font', 'madxartwork-pro' ),
			'not_found_in_trash' => _x( 'No Font found in Trash', 'madxartwork Font', 'madxartwork-pro' ),
			'parent_item_colon' => '',
			'menu_name' => _x( 'Custom Fonts', 'madxartwork Font', 'madxartwork-pro' ),
		];
		...
		$taxonomy_labels = [
			'name' => _x( 'Font Types', 'Font type taxonomy general name', 'madxartwork-pro' ),
			'singular_name' => _x( 'Font Type', 'Font type singular name', 'madxartwork-pro' ),
			'search_items' => __( 'Search Font Types', 'madxartwork-pro' ),
			'popular_items' => __( 'Popular Font Types', 'madxartwork-pro' ),
			'all_items' => __( 'All Font Types', 'madxartwork-pro' ),
			'edit_item' => __( 'Edit Font Type', 'madxartwork-pro' ),
			'update_item' => __( 'Update Font Type', 'madxartwork-pro' ),
			'add_new_item' => __( 'Add New Font Type', 'madxartwork-pro' ),
			'new_item_name' => __( 'New Font Type Name', 'madxartwork-pro' ),
			'separate_items_with_commas' => __( 'Separate Font Types with commas', 'madxartwork-pro' ),
			'add_or_remove_items' => __( 'Add or remove Font Types', 'madxartwork-pro' ),
			'choose_from_most_used' => __( 'Choose from the most used Font Types', 'madxartwork-pro' ),
			'not_found' => __( 'No Font Types found.', 'madxartwork-pro' ),
			'menu_name' => __( 'Font Types', 'madxartwork-pro' ),
		];
	...
	public function post_updated_messages( $messages ) {
		$messages[ self::CPT ] = [
			...
			1 => __( 'Font updated.', 'madxartwork-pro' ),
			2 => __( 'Custom field updated.', 'madxartwork-pro' ),
			3 => __( 'Custom field deleted.', 'madxartwork-pro' ),
			4 => __( 'Font updated.', 'madxartwork-pro' ),
			/* translators: %s: date and time of the revision */
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Font restored to revision from %s', 'madxartwork-pro' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __( 'Font saved.', 'madxartwork-pro' ),
			7 => __( 'Font saved.', 'madxartwork-pro' ),
			8 => __( 'Font submitted.', 'madxartwork-pro' ),
			9 => __( 'Font updated.', 'madxartwork-pro' ),
			10 => __( 'Font draft updated.', 'madxartwork-pro' ),
		];
	...
	public function register_admin_menu() {
		$menu_title = _x( 'Custom Fonts', 'madxartwork Font', 'madxartwork-pro' );
		add_submenu_page(
			Settings::PAGE_ID,
			$menu_title,
			$menu_title,
	...
	public function redirect_admin_old_page_to_new() {
		if ( ! empty( $_GET['page'] ) && 'madxartwork_custom_fonts' === $_GET['page'] ) {
	...
	public function update_enter_title_here( $title, $post ) {
		if ( isset( $post->post_type ) && self::CPT === $post->post_type ) {
			return __( 'Enter Font Family', 'madxartwork-pro' );
		}
	...
	public function manage_columns( $columns ) {
		return [
			...
			'title' => __( 'Font Family', 'madxartwork-pro' ),
			'font_preview' => __( 'Preview', 'madxartwork-pro' ),
		];
	}
	...
	/**
	 * runs on madxartwork font post save and calls the font type handler save meta method
	 *
	 * @param int      $post_id
	 * @param \WP_Post $post
	 * @param bool     $update
	...
	public function add_finder_item( array $categories ) {
		$categories['settings']['items']['custom-fonts'] = [
			'title' => __( 'Custom Fonts', 'madxartwork-pro' ),
			'icon' => 'typography',
			'url' => admin_url( 'edit.php?post_type=' . self::CPT ),
			'keywords' => [ 'custom', 'fonts', 'madxartwork' ],
		];
	...
	protected function actions() {
		...
		add_filter( 'madxartwork/fonts/groups', [ $this, 'register_fonts_groups' ] );
		add_filter( 'madxartwork/fonts/additional_fonts', [ $this, 'register_fonts_in_control' ] );
		add_filter( 'madxartwork/finder/categories', [ $this, 'add_finder_item' ] );
		add_action( 'madxartwork/css-file/post/parse', [ $this, 'enqueue_fonts' ] );
		add_action( 'madxartwork/css-file/global/parse', [ $this, 'enqueue_fonts' ] );
		...
		/**
		 * madxartwork fonts manager loaded.
		 *
		 ...
		 */
		do_action( 'madxartwork_pro/fonts_manager_loaded', $this );
	}
