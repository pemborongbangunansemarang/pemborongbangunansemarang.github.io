...
class Media_Carousel extends Base {
	...
	public function get_title() {
		return __( 'Media Carousel', 'madxartwork-pro' );
	}
	...
	protected function render() {
		...
		if ( $settings['overlay'] ) {
			$this->add_render_attribute( 'image-overlay', 'class', [
				'madxartwork-carousel-image-overlay',
		...
		$settings['container_class'] = 'madxartwork-thumbnails-swiper';
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Lightbox', 'madxartwork-pro' ),
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'#madxartwork-lightbox-slideshow-{{ID}}' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			'lightbox_ui_color',
			[
				'label' => __( 'UI Color', 'madxartwork-pro' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'#madxartwork-lightbox-slideshow-{{ID}} .dialog-lightbox-close-button, #madxartwork-lightbox-slideshow-{{ID}} .madxartwork-swiper-button' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => __( 'UI Hover Color', 'madxartwork-pro' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'#madxartwork-lightbox-slideshow-{{ID}} .dialog-lightbox-close-button:hover, #madxartwork-lightbox-slideshow-{{ID}} .madxartwork-swiper-button:hover' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => __( 'Video Width', 'madxartwork-pro' ),
				...
				'selectors' => [
					'#madxartwork-lightbox-slideshow-{{ID}} .madxartwork-video-container' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	protected function add_repeater_controls( Repeater $repeater ) {
		$repeater->add_control(
			...
			[
				...
				'options' => [
					...
					'video' => [
						'title' => __( 'Video', 'madxartwork-pro' ),
		...
		$repeater->add_control(
			'video',
			[
				'label' => __( 'Video Link', 'madxartwork-pro' ),
				'type' => Controls_Manager::URL,
				'placeholder' => __( 'Enter your video link', 'madxartwork-pro' ),
				'description' => __( 'YouTube or Vimeo link', 'madxartwork-pro' ),
				'show_external' => false,
				'condition' => [
					'type' => 'video',
				],
	...
	protected function print_slide( array $slide, array $settings, $element_key ) {
		if ( ! empty( $settings['thumbs_slider'] ) ) {
			...
			$this->add_render_attribute( $element_key . '-image', 'class', 'madxartwork-fit-aspect-ratio' );
		}

		$this->add_render_attribute( $element_key . '-image', [
			'class' => 'madxartwork-carousel-image',
		...
		if ( $image_link_to && empty( $settings['thumbs_slider'] ) ) {
			...
			if ( 'custom' === $slide['image_link_to_type'] ) {
				...
				$this->add_render_attribute( $element_key . '_link', [
					...
					'data-madxartwork-lightbox-index' => $this->lightbox_slide_index,
				] );
			...
			if ( 'video' === $slide['type'] && $slide['video']['url'] ) {
				...
				$this->add_render_attribute( $element_key . '_link', 'data-madxartwork-lightbox-video', Embed::get_embed_url( $slide['video']['url'], $embed_url_params ) );
			}
	...
	protected function print_slide_image( array $slide, $element_key, array $settings ) {
		...
		<div <?php echo $this->get_render_attribute_string( $element_key . '-image' ); ?>>
			<?php if ( 'video' === $slide['type'] && $settings['video_play_icon'] ) : ?>
				<div class="madxartwork-custom-embed-play">
					<i class="eicon-play" aria-hidden="true"></i>
					<span class="madxartwork-screen-only"><?php _e( 'Play', 'madxartwork-pro' ); ?></span>
				</div>
			<?php endif; ?>
		</div>
	...
	private function add_injections() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'carousel',
				'options' => [
					'carousel' => __( 'Carousel', 'madxartwork-pro' ),
					'slideshow' => __( 'Slideshow', 'madxartwork-pro' ),
					'coverflow' => __( 'Coverflow', 'madxartwork-pro' ),
				],
				'prefix_class' => 'madxartwork-skin-',
		...
		$this->add_control(
			'image_fit',
			[
				'label' => __( 'Image Fit', 'madxartwork-pro' ),
				'type' => Controls_Manager::SELECT,
				'default' => '',
				'options' => [
					'' => __( 'Cover', 'madxartwork-pro' ),
					'contain' => __( 'Contain', 'madxartwork-pro' ),
					'auto' => __( 'Auto', 'madxartwork-pro' ),
				],
				'selectors' => [
					'{{WRAPPER}} .madxartwork-main-swiper .madxartwork-carousel-image' => 'background-size: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			'play_icon_title',
			[
				'label' => __( 'Play Icon', 'madxartwork-pro' ),
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-custom-embed-play i' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .madxartwork-custom-embed-play i' => 'font-size: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-custom-embed-play i',
				'fields_options' => [
					'text_shadow_type' => [
						'label' => _x( 'Shadow', 'Text Shadow Control', 'madxartwork-pro' ),
					],
				],
			]
		);
		...
		$this->add_control(
			'caption',
			[
				'label' => __( 'Caption', 'madxartwork-pro' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'title',
				'options' => [
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-carousel-image-overlay' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-carousel-image-overlay' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .madxartwork-carousel-image-overlay',
				'condition' => [
					'overlay' => 'text',
				],
			]
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} .madxartwork-carousel-image-overlay i' => 'font-size: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'overlay' => 'icon',
				],
		...
		$this->add_control(
			'thumbs_title',
			[
				'label' => __( 'Thumbnails', 'madxartwork-pro' ),
				'type' => Controls_Manager::HEADING,
				'condition' => [
					'skin' => 'slideshow',
				],
		...
		$this->add_control(
			'thumbs_ratio',
			[
				'label' => __( 'Ratio', 'madxartwork-pro' ),
				'type' => Controls_Manager::SELECT,
				'default' => '219',
				'options' => [
					'169' => '16:9',
		...
		$this->add_control(
			'centered_slides',
			[
				'label' => __( 'Centered Slides', 'madxartwork-pro' ),
	...
	private function update_controls() {
		...
		$this->update_responsive_control(
			...
			[
				'selectors' => [
					'{{WRAPPER}}.madxartwork-skin-slideshow .madxartwork-main-swiper' => 'margin-bottom: {{SIZE}}{{UNIT}}',
				],
				'render_type' => 'ui',
			]
		);
