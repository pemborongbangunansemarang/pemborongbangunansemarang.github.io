<?php
namespace madxartworkPro\Modules\ThemeBuilder;

use madxartwork\Core\Base\Document;
use madxartwork\Elements_Manager;
use madxartwork\TemplateLibrary\Source_Local;
use madxartworkPro\Base\Module_Base;
use madxartworkPro\Classes\Utils;
use madxartworkPro\Modules\ThemeBuilder\Classes;
use madxartworkPro\Modules\ThemeBuilder\Documents\Single;
...
class Module extends Module_Base {
	...
	public static function is_preview() {
		return Plugin::madxartwork()->preview->is_preview_mode() || is_preview();
	}
	...
	public function get_widgets() {
		...
		if ( class_exists( '\madxartworkPro\Modules\Posts\Widgets\Posts' ) ) {
	...
	public function localize_settings( $settings ) {
		...
		$settings = array_replace_recursive( $settings, [
			'i18n' => [
				'publish_settings' => __( 'Publish Settings', 'madxartwork-pro' ),
				'conditions' => __( 'Conditions', 'madxartwork-pro' ),
				'display_conditions' => __( 'Display Conditions', 'madxartwork-pro' ),
				'choose' => __( 'Choose', 'madxartwork-pro' ),
				'add_condition' => __( 'Add Condition', 'madxartwork-pro' ),
				'conditions_title' => sprintf( __( 'Where Do You Want to Display Your %s?', 'madxartwork-pro' ), $document->get_post_type_title() ),
				'conditions_description' => sprintf( __( 'Set the conditions that determine where your %s is used throughout your site.', 'madxartwork-pro' ), $document->get_post_type_title() ) . '<br>' . __( 'For example, choose \'Entire Site\' to display the template across your site.', 'madxartwork-pro' ),
				'conditions_publish_screen_description' => __( 'Apply current template to these pages.', 'madxartwork-pro' ),
				'save_and_close' => __( 'Save & Close', 'madxartwork-pro' ),
			],
			'theme_builder' => [
	...
	public function register_controls() {
		$controls_manager = Plugin::madxartwork()->controls_manager;
	...
	public function create_new_dialog_types( $types ) {
		...
		foreach ( $types as $type => $label ) {
			$document_type = Plugin::madxartwork()->documents->get_document_type( $type );
	...
	public function print_location_field() {
		...
		?>
		<div id="madxartwork-new-template__form__location__wrapper" class="madxartwork-form-field">
			<label for="madxartwork-new-template__form__location" class="madxartwork-form-field__label">
				<?php echo __( 'Select a Location', 'madxartwork-pro' ); ?>
			</label>
			<div class="madxartwork-form-field__select__wrapper">
				<select id="madxartwork-new-template__form__location" class="madxartwork-form-field__select" name="meta_location">
					<option value="">
						<?php echo __( 'Select...', 'madxartwork-pro' ); ?>
					</option>
					<?php
	...
	public function print_post_type_field() {
		...
		?>
		<div id="madxartwork-new-template__form__post-type__wrapper" class="madxartwork-form-field">
			<label for="madxartwork-new-template__form__post-type" class="madxartwork-form-field__label">
				<?php echo __( 'Select Post Type', 'madxartwork-pro' ); ?>
			</label>
			<div class="madxartwork-form-field__select__wrapper">
				<select id="madxartwork-new-template__form__post-type" class="madxartwork-form-field__select" name="<?php echo Single::REMOTE_CATEGORY_META_KEY; ?>">
					<option value="">
						<?php echo __( 'Select', 'madxartwork-pro' ); ?>...
					</option>
					<?php
					...
					foreach ( $post_types as $post_type => $label ) {
						$doc_type = Plugin::madxartwork()->documents->get_document_type( $post_type );
						$doc_class = new $doc_type();

						// New: Core >=2.7.0
						$is_base_page = class_exists( '\madxartwork\Core\DocumentTypes\PageBase' ) && $doc_class instanceof \madxartwork\Core\DocumentTypes\PageBase;
					...
					// 404.
					echo sprintf( '<option value="%1$s">%2$s</option>', 'not_found404', __( '404 Page', 'madxartwork-pro' ) );
	...
	public function admin_head() {
		...
		if ( $current_screen && in_array( $current_screen->id, [ 'madxartwork_library', 'edit-madxartwork_library' ] ) ) {
	...
	public function on_madxartwork_editor_init() {
		Plugin::madxartwork()->common->add_template( __DIR__ . '/views/panel-template.php' );
	}

	public function add_finder_items( array $categories ) {
		$categories['general']['items']['theme-builder'] = [
			'title' => __( 'Theme Builder', 'madxartwork-pro' ),
			'icon' => 'library-save',
		...
		$categories['create']['items']['theme-template'] = [
			'title' => __( 'Add New Theme Template', 'madxartwork-pro' ),
			'icon' => 'plus-circle-o',
	...
	public function admin_menu() {
		add_submenu_page( Source_Local::ADMIN_MENU_SLUG, '', __( 'Theme Builder', 'madxartwork-pro' ), 'publish_posts', $this->get_admin_templates_url( true ) );
	}
	...
	public function __construct() {
		...
		add_action( 'madxartwork/template-library/create_new_dialog_fields', [ $this, 'print_location_field' ] );
		add_action( 'madxartwork/template-library/create_new_dialog_fields', [ $this, 'print_post_type_field' ] );
		add_filter( 'madxartwork/template-library/create_new_dialog_types', [ $this, 'create_new_dialog_types' ] );
