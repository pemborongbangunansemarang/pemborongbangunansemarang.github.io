...
use madxartworkPro\Modules\Forms\Classes\Form_Record;
use madxartworkPro\Modules\Forms\Classes\Integration_Base;
use madxartworkPro\Modules\Forms\Controls\Fields_Map;
use madxartworkPro\Modules\Forms\Classes\Mailchimp_Handler;
...
class Mailchimp extends Integration_Base {
	...
	private function get_global_api_key() {
		return get_option( 'madxartwork_' . self::OPTION_NAME_API_KEY );
	}
	...
	public function get_label() {
		return __( 'MailChimp', 'madxartwork-pro' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_mailchimp',
			[
				'label' => __( 'MailChimp', 'madxartwork-pro' ),
				'condition' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'API Key', 'madxartwork-pro' ),
		...
		$widget->add_control(
			'mailchimp_api_key',
			[
				'label' => __( 'Custom API Key', 'madxartwork-pro' ),
				'type' => Controls_Manager::TEXT,
				'condition' => [
					'mailchimp_api_key_source' => 'custom',
				],
				'description' => __( 'Use this field to set a custom API Key for the current form', 'madxartwork-pro' ),
			]
		);
		...
		$widget->add_control(
			'mailchimp_list',
			[
				'label' => __( 'Audience', 'madxartwork-pro' ),
				'type' => Controls_Manager::SELECT,
				'options' => [],
				'render_type' => 'none',
				'conditions' => [
		...
		$widget->add_control(
			'mailchimp_groups',
			[
				'label' => __( 'Groups', 'madxartwork-pro' ),
		...
		$widget->add_control(
			'mailchimp_tags',
			[
				'label' => __( 'Tags', 'madxartwork-pro' ),
				'description' => __( 'Add comma separated tags', 'madxartwork-pro' ),
				'type' => Controls_Manager::TEXT,
				'render_type' => 'none',
				'condition' => [
					'mailchimp_list!' => '',
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Double Opt-In', 'madxartwork-pro' ),
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Field Mapping', 'madxartwork-pro' ),
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'mailchimp', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'MailChimp', 'madxartwork-pro' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'madxartwork-pro' ), 'https://kb.mailchimp.com/integrations/api-integrations/about-api-keys' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button madxartwork-button-spinner" id="madxartwork_pro_mailchimp_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'madxartwork-pro' ) ),
					],
				],
			],
		] );
	...
	public function __construct() {
		if ( is_admin() ) {
			add_action( 'madxartwork/admin/after_create_settings/' . Settings::PAGE_ID, [ $this, 'register_admin_fields' ], 14 );
		}
