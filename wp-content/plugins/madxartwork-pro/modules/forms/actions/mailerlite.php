...
use madxartworkPro\Modules\Forms\Classes\Mailerlite_Handler;
...
class Mailerlite extends Integration_Base {
	...
	public function get_label() {
		return __( 'MailerLite', 'madxartwork-pro' );
	}
	...
	public function register_settings_section( $widget ) {
		$widget->start_controls_section(
			'section_mailerlite',
			[
				'label' => __( 'MailerLite', 'madxartwork-pro' ),
				'condition' => [
		...
		$widget->add_control(
			'mailerlite_group',
			[
				'label' => __( 'Group', 'madxartwork-pro' ),
				'type' => Controls_Manager::SELECT,
				'options' => [],
				'render_type' => 'none',
				'conditions' => [
		...
		$widget->add_control(
			...
			[
				'label' => __( 'Email Field Mapping', 'madxartwork-pro' ),
		...
		$widget->add_control(
			'allow_resubscribe',
			[
				'label' => __( 'Allow Resubscribe', 'madxartwork-pro' ),
	...
	public function run( $record, $ajax_handler ) {
		...
		if ( ! $subscriber ) {
			$ajax_handler->add_admin_error_message( __( 'MailerLite Integration requires an email field', 'madxartwork-pro' ) );
			return;
		}
	...
	public function register_admin_fields( Settings $settings ) {
		$settings->add_section( Settings::TAB_INTEGRATIONS, 'mailerlite', [
			'callback' => function() {
				echo '<hr><h2>' . esc_html__( 'MailerLite', 'madxartwork-pro' ) . '</h2>';
			},
			'fields' => [
				self::OPTION_NAME_API_KEY => [
					...
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'To integrate with our forms you need an <a href="%s" target="_blank">API Key</a>.', 'madxartwork-pro' ), 'https://help.mailerlite.com/article/show/35040-where-can-i-find-the-api-key' ),
					],
				],
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-action="%s" data-nonce="%s" class="button madxartwork-button-spinner" id="madxartwork_pro_mailerlite_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce( self::OPTION_NAME_API_KEY ), __( 'Validate API Key', 'madxartwork-pro' ) ),
					],
				],
			],
		] );
