...
/*
  ...
  Version: v1.0
  Plugin URI: http://shop.MadxArtwork.net
...
class Admin{
    ...
    const AEPRO_PLUGIN_LICENSE_SERVER_URL = '';
    const AEPRO_PLUGIN_ITEM_REFERENCE = 'AnyWhere madxartwork Pro';
    ...
    public function ae_admin_notices(){
        ...
        if($license_status == 'missing'){
            $msg = sprintf( __( '<strong>AnyWhere madxartwork Pro licence is missing</strong><br/>Please <a href="%s">add your license key</a> to enable automatic update notifications.', 'ae-pro' ), $license_page_link );
            printf( '<div class="error"><p>%s</p></div>', $msg );
            return;
        }elseif($license_status == 'invalid'){
            $msg = sprintf( __( '<strong>AnyWhere madxartwork Pro licence is invalid</strong><br/>Please <a href="%s">add your license key</a> to enable automatic update notifications.', 'ae-pro' ), $license_page_link );
            printf( '<div class="error"><p>%s</p></div>', $msg );
            return;
        }
...
define('AEPRO_PLUGIN_LICENSE_SERVER_URL', ''); //Rename this constant name so it is specific to your plugin or theme.
...
function ae_pro_license_management_page() {
    ...
    if (isset($_REQUEST['activate_license'])) {
        ...
        if($license_data->result == 'success'){//Success was returned for the license activation
            ...
            $msg = sprintf( __( '<strong>Error in activating licence for AnyWhere madxartwork Pro</strong> <br/> Reason: %s', 'ae-pro' ), $license_data->message );
            printf( '<div class="error"><p>%s</p></div>', $msg );
            //Uncomment the followng line to see the message that returned from the license server
           // echo '<br />The following message was returned from the server: '.$license_data->message;
        }
    ...
    if (isset($_REQUEST['deactivate_license'])) {
        ...
        if($license_data->result == 'success'){//Success was returned for the license activation
            ...
            //Uncomment the followng line to see the message that returned from the license server
            $msg = sprintf( __( '<strong>Error in de-activating licence key for AnyWhere madxartwork Pro</strong> <br/> Reason: %s', 'ae-pro' ), $license_data->message );
            printf( '<div class="error"><p>%s</p></div>', $msg );
            //echo '<br />The following message was returned from the server: '.$license_data->message;
