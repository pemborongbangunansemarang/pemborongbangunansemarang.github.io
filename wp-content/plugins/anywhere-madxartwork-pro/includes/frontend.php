...
class Frontend{
    ...
    private function __construct() {
        ...
        add_shortcode('INSERT_madxartwork',[ $this, 'render_shortcode' ]);
    ...
    public function render_shortcode($atts){
        ...
        return $this->render_insert_madxartwork($template_id);

    }

    public function run_madxartwork_builder($template_id){
        ...
        if(Plugin::$instance->db->is_built_with_madxartwork( $post_id )) {
            ?>
            <div class="ae_data madxartwork madxartwork-<?php echo $post_id; ?>" data-aetid="<?php echo $post_id; ?>">
                <?php echo madxartwork\Plugin::instance()->frontend->get_builder_content_for_display( $post_id ); ?>
            </div>
            <?php
        }else{
            echo __('Not a valid madxartwork page','ae-pro');
        }
    ...
    public function render_insert_madxartwork($template_id,$with_css = false){
        if(!isset($template_id) || empty($template_id)){
            return '';
        }
        ...
        // check if page is madxartwork page

        $edit_mode = get_post_meta($post_id,'_madxartwork_edit_mode','');
        ...
        if(Plugin::$instance->db->is_built_with_madxartwork( $post_id )) {
            ...
            <div class="ae_data madxartwork madxartwork-<?php echo $post_id; ?>" data-aetid="<?php echo $post_id; ?>">
                <?php
                    ...
                    echo madxartwork\Plugin::instance()->frontend->get_builder_content( $post_id,$with_css );

                    add_action('wp_footer', function(){

                        if(is_archive()){
                            $madxartwork_frontend_config['settings'] = $this->get_init_settings();
                            wp_localize_script( 'madxartwork-frontend', 'madxartworkFrontendConfig', $madxartwork_frontend_config['settings'] );

                            //echo '<pre>'; print_r($madxartwork_frontend_config); die();
    ...
    private function print_ae_data($sections){
        foreach ( $sections as $section_data ) {
            $section = new madxartwork\Element_Section( $section_data );
            ...
            while($templates->have_posts()){
                ...
                if(!empty($hook_position)){
                    ...
                    add_action($hook_position,function(){
                        ...
                        echo $this->render_insert_madxartwork($this->_hook_templates[$current_filter]);
                    },10,1);
                }
            }
            wp_reset_postdata();
    ...
    public function apply_ae_single_post_template($content){
        ...
        if(isset($ae_post_template) && is_numeric($ae_post_template)){
            $template_content = $this->render_insert_madxartwork($ae_post_template);
    ...
    public function apply_ae_archive_template($content){
        ...
        if($ae_archive_template){
            $template_content = $this->render_insert_madxartwork($ae_archive_template);
            echo $template_content;
        }
    ...
    public function apply_ae_wc_single_template(){
        ...
        if($ae_product_template != '' && is_numeric($ae_product_template)){
            $template_content = $this->render_insert_madxartwork($ae_product_template);
    ...
    function apply_404_template(){
        ...
        if($tid){
            echo $this->render_insert_madxartwork($tid);
        }
    }
	 ...
	 * @return array
     *
     * Copied for madxartwork Frontend as that was protected
	 */
