...
class Aepro_Bg_Slider {
	...
	private function __construct() {
		add_action('madxartwork/element/before_section_start',[ $this, '_add_controls'],10,3);

		add_action( 'madxartwork/frontend/element/before_render', [ $this, '_before_render'],10,1);

		add_action( 'madxartwork/frontend/column/before_render', [ $this, '_before_render'],10,1);
		add_action( 'madxartwork/frontend/section/before_render', [ $this, '_before_render'],10,1);

		add_action( 'madxartwork/element/print_template', [ $this, '_print_template'],10,2);
		add_action( 'madxartwork/section/print_template', [ $this, '_print_template'],10,2);
		add_action( 'madxartwork/column/print_template', [ $this, '_print_template'],10,2);
	...
	public function _add_controls( $element, $section_id, $args ) {
		if ( ( 'section' === $element->get_name() && 'section_advanced' === $section_id ) || ( 'column' === $element->get_name() && 'section_advanced' === $section_id ) ) {
			...
			$element->start_controls_section(
				...
				[
					'label' => __( 'Background Slider', 'madxartwork' ),
	...
	function _before_render( \madxartwork\Element_Base $element ) {
        ...
        <script type="text/javascript">
            jQuery(document).ready(function () {
                jQuery(".madxartwork-element-<?php echo $element->get_id(); ?>").prepend('<div class="aepro-section-bs"><div class="aepro-section-bs-inner"></div></div>');
                ...
                if ('<?php echo $settings["custom_overlay_switcher"]; ?>' == 'yes') {
                    ...
                    //}else{
                    var bgoverlay = '<?php echo plugins_url() . "/anywhere-madxartwork-pro/includes/assets/lib/vegas/overlays/00.png"; ?>';
                    // }
                } else {
                    if ('<?php echo $settings["slider_overlay"]; ?>') {
                        var bgoverlay = '<?php echo plugins_url() . "/anywhere-madxartwork-pro/includes/assets/lib/vegas/overlays/" . $settings["slider_overlay"] . ".png"; ?>';
                    } else {
                        var bgoverlay = '<?php echo plugins_url() . "/anywhere-madxartwork-pro/includes/assets/lib/vegas/overlays/00.png"; ?>';
                    }
                }
                ...
                jQuery(".madxartwork-element-<?php echo $element->get_id(); ?>").children('.aepro-section-bs').children('.aepro-section-bs-inner').vegas({
                ...
                if ('<?php echo $settings["custom_overlay_switcher"]; ?>' == 'yes') {
                    jQuery(".madxartwork-element-<?php echo $element->get_id(); ?>").children('.aepro-section-bs').children('.aepro-section-bs-inner').children('.vegas-overlay').css('background-image', '');
                }
            });
        </script>
		<?php
	...
	function _print_template( $template, $widget ) {
        ...
        slides_path_string = ['<?php echo plugins_url('anywhere-madxartwork-pro') . '/includes/assets/images/aep-placeholder.jpg' ?>', '<?php echo plugins_url('anywhere-madxartwork-pro') . '/includes/assets/images/aep-placeholder.jpg' ?>'].join();
        }
        #>
