...
class Aepro{
    ...
    public function init(){
        ...
        add_post_type_support( 'ae_global_templates', 'madxartwork' );
    ...
    private function __construct() {
        ...
        add_action( 'madxartwork/init',[ $this, 'madxartwork_loaded']);

        // for frontend scripts & styles
        add_action( 'wp_enqueue_scripts', [ $this, '_enqueue_scripts' ]);

        // madxartwork editor scripts & styles
        add_action('madxartwork/editor/wp_head', [$this, '_editor_enqueue_scripts']);
        ...
        add_action( 'madxartwork/widgets/widgets_registered', [$this, 'madxartwork_widget_registered']);
	...
	public function ae_template_canvas($template){
		if ( is_singular('ae_global_templates') ) {
			...
			if ($helper->is_canvas_enabled(get_the_ID())) {
				$template = madxartwork_PATH . '/modules/page-templates/templates/canvas.php';
				return $template;
			}

			if($helper->is_heder_footer_enabled(get_the_ID())){
				$template = madxartwork_PATH . '/modules/page-templates/templates/header-footer.php';
				return $template;
			}
		}
    ...
    function _editor_enqueue_scripts(){
        ...
        wp_enqueue_script('ae-madxartwork-editor-js',AE_PRO_URL.'includes/assets/js/common'. AE_PRO_SCRIPT_SUFFIX .'.js', array('jquery','ae-gmap'), AE_PRO_VERSION);

        $localize_data = array(
            'plugin_url' => plugins_url('anywhere-madxartwork-pro')
        );
        wp_localize_script( 'ae-madxartwork-editor-js', 'aepro_editor', $localize_data );
   ...
   public function set_custom_edit_ae_global_templates_posts_columns($columns) {
    ...
    public function add_ae_global_templates_columns( $column, $post_id ) {
        ...
        switch ( $column ) {
            ...
            case 'ae_shortcode_column' :
                echo '<input type=\'text\' class=\'widefat\' value=\'[INSERT_madxartwork id="'.$post_id.'"]\' readonly="">';
                break;
    ...
    public function _plugins_loaded(){
        ...
        if(!isset($_REQUEST['action']) || $_REQUEST['action'] != 'madxartwork'){
    ...
    private function _includes(){
        ...
        if($this->licence_activated()){
            ...
            //new AE_Updater( 'http://wp-updates.com/api/2/plugin',plugin_basename(AE_PRO_PATH.'anywhere-madxartwork-pro.php') );
        }
    ...
    public function madxartwork_loaded(){
	    ...
	    add_action( 'madxartwork/finder/categories/init', function( $categories_manager ) {
		    // Add the category
    ...
    public function madxartwork_widget_registered()
    {
	...
	public function db_upgrade_script(){
		...
		if($upgrade_required != 1){
			...
			if(count($posts)){
				...
				// set new meta key
				foreach($posts as $p){
					update_post_meta($p->ID, 'ae_madxartwork_template', 'ec');
				}
			}
	...
	public function ae_pro_fail_load() {
		...
		if ( _is_madxartwork_installed() ) {
			...
			$message = sprintf( __( '<b>AnyWhere madxartwork Pro</b> is not working because you need to activate the <b>madxartwork</b> plugin.', 'aepro' ), '<strong>', '</strong>' );
			$action_url   = wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . $plugin . '&amp;plugin_status=all&amp;paged=1&amp;s', 'activate-plugin_' . $plugin );
			$button_label = __( 'Activate madxartwork', 'aepro' );

		} else {
			if ( ! current_user_can( 'install_plugins' ) ) {
				return;
			}
			$message = sprintf( __( '<b>AnyWhere madxartwork Pro</b> is not working because you need to install the <b>madxartwork</b> plugin.', 'aepro' ), '<strong>', '</strong>' );
			$action_url   = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=madxartwork' ), 'install-plugin_madxartwork' );
			$button_label = __( 'Install madxartwork', 'aepro' );
		}
